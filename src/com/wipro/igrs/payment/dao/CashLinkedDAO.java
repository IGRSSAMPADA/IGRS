 ...
 * ===========================================================================
 */
public class CashLinkedDAO {
	ArrayList list = null;
	String query;
	DBUtility dbUtil=null; // obj for dbutility
	private Logger logger = (Logger) Logger.getLogger(CashLinkedDAO.class);
	private CallableStatement clstmt;
	public CashLinkedDAO() throws Exception {
	...
	public String getCashReceipt(CashCounterLinkedForm cashform) throws Exception{
        ...
        String []arry=new String[4];
          arry[0] = cashform.getCheckerRegInitId();
        arry[1]= "A";
        arry[2]= cashform.getListService();
        arry[3] = cashform.getServFee();
		try {
			dbUtil = new DBUtility();
			
			dbUtil.createPreparedStatement(CommonSQL.Cash_receipt_new);
			mainlist=dbUtil.executeQry(arry);
     ...
     */
	 ...
	 public boolean getCashExistingDetail(String[] param)throws Exception{
	    	...
	    	try {
	    		...
	    		//DBUtility db = new DBUtility();
	    		dbUtil.setAutoCommit(false);
	    		dbUtil.createPreparedStatement(CommonSQL.ACTIVE_CASH);
     ...
     */
	 ...
	 /**
	     ...
	     */
		 public boolean addFinalCashDetails(CashCounterLinkedForm form, HashMap map, String uid)throws Exception{
				...
				/*if(form.getListService()!=null){
				cashdto1.setListService(form.getListService());
				}*/
				try {
					dbUtil = new DBUtility();
					dbUtil.setAutoCommit(false);
		    		for(int n=0;n<objary.length;n++){
					cashdto1=(CashCounterLinkedDTO)objary[n];
					...
					//String txnid	  =	form.getChallanDTO().getTxnid();
					 ...
					 String regTxnId = form.getCheckerRegInitId();
					//akansha
		    		...
		    		//DBUtility db = new DBUtility();
		    		dbUtil.createPreparedStatement(CommonSQL.CASH_INSERT_New);
	 ...
	 /**
		...
		public ArrayList getRevHeadsFee(String serviceId) throws Exception{
		 ...
		 public ArrayList getDetails(CashCounterLinkedForm form){
...
//akansha
		public String getTransactionid(String combid) throws Exception{
				...
				try {
					...
					dbUtil.createPreparedStatement(CommonSQL.TRANS_QUERY_NEW);
					mainlist=dbUtil.executeQry(arry);
		...
		public String getCombineId(String receipt) throws Exception{
			...
			try {
				...
				dbUtil.createPreparedStatement(CommonSQL.CombineID);
				mainlist=dbUtil.executeQry(arry);
		...
		public String getServiceEng(String functionId) {
			...
			try {
				...
				dbUtil.createPreparedStatement(CommonSQL.CASH_FUNCTIONID_SERVICEEng);
				functionName=dbUtil.executeQry(arry);
				logger.debug("the values in list is  " + list);
			} catch (Exception x) {
				logger.error("Exception in funcidlist(): " + x);
...
public String getServiceHnd(String functionId) {
			...
			try {
				...
				dbUtil.createPreparedStatement(CommonSQL.CASH_FUNCTIONID_SERVICEHnd);
				functionName=dbUtil.executeQry(arry);
				logger.debug("the values in list is  " + list);
			} catch (Exception x) {
				logger.error("Exception in funcidlist(): " + x);
