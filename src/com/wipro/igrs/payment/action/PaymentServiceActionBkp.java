...
public class PaymentServiceActionBkp extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (form != null) {
			...
			if (param != null && param.equalsIgnoreCase("PaymentServiceHome")) {
				...
				ArrayList districtList = bd.getDistrictList(languageLocale);
				...
				pForm.setUserValue("");
				pForm.setUserkey("");
				pForm.setDistrictList(districtList);
				pForm.setSlectedDictrictID("");
			...
			if (fwdName != null
				...
				funcID= (String)session.getAttribute("payFuncID")==null?funcID:(String)session.getAttribute("payFuncID");
				refID = (String)session.getAttribute("referenceID")==null?refID:(String)session.getAttribute("referenceID");
				payableAmt = (String)session.getAttribute("payableAmt")==null?payableAmt:(String)session.getAttribute("payableAmt");
			...
			if (fwdName != null
				...
				JrxmlExportUtility export = new JrxmlExportUtility();
				String functionId = "";
				if(null!=request.getParameter("payFuncID")){
					functionId = request.getParameter("payFuncID");
				}
				String reportPath="";
				HashMap jasperMap = new HashMap();
				if(functionId.equals("FUN_501")){
					 reportPath = path.concat(CommonConstant.PAYMENT_RECEIPT_EFILE);
					jasperMap.put("function_id", functionId);
					jasperMap.put("refID", request.getParameter("referenceID"));
					jasperMap.put("lang", languageLocale);
					jasperMap.put("Amount", request.getParameter("payableAmt"));
					jasperMap.put("UserID", userId);
					jasperMap.put("receiptDate", new java.util.Date());
					jasperMap.put("path", path);
				}else{
					reportPath = path
