					...
					if (Integer.parseInt(claimantArr[1].trim()) == RegInitConstant.POA_HOLDER_FLAG) {
						...
						if (partyAge < 18) {
							...
							if (languageLocale.equalsIgnoreCase(RegInitConstant.LANGUAGE_ENGLISH)) {
								...
								name = "(अवयस्‍क) " + name + " " + relations + " " + regForm.getFatherNameTrns() + " अभिभावक  " + regForm.getGuardianTrans();
							}
							/*
						...
						// Initiation - Rahul
							...
							if (partyAge < 18) {
								...
								if (languageLocale.equalsIgnoreCase(RegInitConstant.LANGUAGE_ENGLISH)) {
									...
									name = "(अवयस्‍क  ) " + name + " " + relations + " " + regForm.getFatherName() + " द्वारा   " + regForm.getGuardian();
								}
								/*
								 * mapDto.setAgeAboveName(name);
								 * regForm.setAbc("1");
			...
			if (RegInitConstant.NEXT_TO_PAYMENT_ACTION.equals(actionName)) {
				...
				if (!rebateApplicableCheck) {
					if (languageLocale.equalsIgnoreCase("en")) {
						...
						request.setAttribute("ERROR", "सभी खरीदारों को महिला होना चाहिए क्योंकि छूट ली गई है / संगठन या सरकार के लिए छूट");
					}
