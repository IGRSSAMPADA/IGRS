...
public class updateTreasuryAction extends BaseAction {
    
	private Logger logger = (Logger) Logger.getLogger(updateTreasuryAction.class);
	private TreasureBD treasureBD;
	private TreasureDTO dto; 
	private TreasuryForm treasuryForm;
	ArrayList treasuryList=null;
    public updateTreasuryAction() 
    {
			...
			
	{
		...
		{
			setList(request);
			return mapping.findForward("viewTreasuryMaster2");
		}
		/* Update Treasury master */
		dto.setTreasuryId(treasuryForm.getTreasuryId());
		dto.setStatus(treasuryForm.getStatus());
		dto.setUpdatedBy(user);
		//treasuryForm.setPageName("");
		try {
			treasureBD.updateTreasurymaster(dto);
		} 
		catch (IllegalTreasuryException e)
		{
			errors = new ActionErrors();
			errors.add("treasury",new ActionError("errors.treasury.usernameexists"));
			saveErrors(request, errors);
			treasuryForm.setPageName("treasuryUpdate");
