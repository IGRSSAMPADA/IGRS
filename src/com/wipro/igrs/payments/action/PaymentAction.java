...
public class PaymentAction extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		...
		if ("payment".equals(param)) {
			...
			eForm.setEnableCash(paymentBD.getEnableCash());
		...
		if (form != null) {
			...
			if (online != null && online.equalsIgnoreCase("onlinePayment")) {
				...
				eForm.setAmountbytrsry(dto.getAmount());
				eForm.setChallandatetrsry(dto.getChallandate());
				eForm.setChecksum(dto.getChecksum());
				// Added by Ajit
				eForm.setStatusTrs(dto.getStatusTrs());
				eForm.setStatusDesc(dto.getStatusDesc());
				...
				LoadMenuExternal.getInstance().deleteOnlinePayementDto(crn);// Removing
				// from
				// Application
				// Context
				try {
			...
			System.out.println("par value ---" + par);
			...
			if ("CR".equalsIgnoreCase(par)) {
				...
				if (session.getAttribute("PayTransId") != null) {
					String trans = (String) session.getAttribute("PayTransId");
					eForm.setTransId(trans);
					String crEntrAmt = paymentBD.getPaidAmount(trans);
					eForm.setCrEntrAmt(crEntrAmt);
				} else {

					String crEntrAmt = paymentBD.getPaidAmount(eForm.getTransId());
					eForm.setCrEntrAmt(crEntrAmt);
		...
		// String loginBy = (String)session.getAttribute("roleName");//Commented
		// by ramesh on 03 dec12
		...
		String formName1;
		String serviceId = "";
		if (request.getAttribute("parentFunId") != null && request.getAttribute("parentAmount") != null && request.getAttribute("parentFunName") != null && request.getAttribute("forwardPath") != null && request.getAttribute("parentKey") != null && request.getAttribute("parentOfficeId") != null && request.getAttribute("parentOfficeName") != null && request.getAttribute("parentDistrictId") != null && request.getAttribute("parentDistrictName") != null && request.getAttribute("parentReferenceId") != null && request.getAttribute("parentModName") != null && request.getAttribute("parentColumnName") != null && request.getAttribute("parentTable") != null) {
			...
			eForm.setServiceId(eForm.getServiceId());
		...
		// String roleId = (String)session.getAttribute("roleId");//Commented by
		// Ramesh
		// String roleIds =
		// (String)session.getAttribute("loggedInEMPDesignation");
		String roleId = paymentBD.getRole(userId); // Added by Aakriti for
		...
		// ---Start------Added by Aakriti----- // these are the fixed roles as
		// told by TL, if roles change, module will not work as expected
		...
		// added by preeti on 20 april 2016 for Public officer role
		String porole = "GR1420652055929";

		// end of changes preeti on 20 april 2016 for Public officer role
		// String getStatus1 = null;
		// ---End------Added by Aakriti-----

		// if(roleId.equalsIgnoreCase(srrole) ||
		// loginBy.equalsIgnoreCase(srrole)) {//Commented by Ramesh
		if (roleId.equalsIgnoreCase(srrole)) {
			jspPage = "SRO";
			String par = request.getParameter("Prnt");
			if (par != null || par != "") {
		...
		// if(roleId.equalsIgnoreCase(sprole) ||
		// loginBy.equalsIgnoreCase(sprole)) {//Commented by Ramesh
			...
			if ("Credit".equalsIgnoreCase(spFlg)) {
				...
				if (par != null || par != "") {
					if ("CH".equalsIgnoreCase(par)) {
						...
						String transDate = sdf2.format(d1);
						// String depoName=
						// eForm.getPayDTO().getDepositorName();
						String depoName = paymentBD.getdepoDtl(eForm);
						String issueName = paymentBD.getissueName(eForm);
						eForm.setActionName("");
						eForm.setFormName("");
		...
		if (roleId.equalsIgnoreCase(rurole) || roleId.equalsIgnoreCase(porole)) {
		...
		if (roleId.equalsIgnoreCase(porole)) {
			jspPage = "RU";
			String par = request.getParameter("Prnt");
			if (par != null & "".equalsIgnoreCase(par)) {
				if ("ON".equalsIgnoreCase(par)) {
					String purpname = paymentBD.getFunctionName(eForm.getFuncId(), languageLocale);
					String trans = (String) session.getAttribute("PayTransId");
					eForm.setTransId(trans);
		...
		System.out.println("ACTION  PAGE..ActionName :- " + actionName + " FormName :- " + formName);
		logger.debug("ACTION  PAGE..ActionName :- " + actionName + " FormName :- " + formName);
		 ...
		 * )) { try { ArrayList spList = new ArrayList(); spList =
		 * paymentBD.getSpDetails(userId);
		 * System.out.println("Size of list"+spList.size()); if(spList.size()>0)
		 * { for(int i=0;i<spList.size();i++) { //SELECT LICENSE_NO,
		 * LICENSE_TXN_ID FROM IGRS_SP_USER_LICENSE_DETAILS ArrayList list
		 * =(ArrayList)spList.get(i);
		 ...
		 * 
		 * } // ----------Commented by Aakriti-------------- String funId =
		 * (String) session.getAttribute("functionId"); String subFunId =
		 * (String) session.getAttribute("subFunId"); if (funId == null) {
		 * session.setAttribute("functionId", "1001"); funId = (String)
		 * session.getAttribute( "functionId"); } String spFee =
		 * paymentBD.getSPFee(funId, forwardJsp);
		 * session.setAttribute("commision", spFee); double amt =
		 * Double.parseDouble(amount); double totamt = amt -
		 * (Double.valueOf(spFee.trim()).doubleValue()); double crdtLimit =
		 * (Double
		 * .valueOf(eForm.getPaymentDTO().getSpAmt().trim()).doubleValue());
		...
		if (request.getParameter("ChallanDownload") != null) {
			...
			if (!(chllnDwnldPage).equalsIgnoreCase("")) {// DOWNLOAD CHALLAN
				if (chllnDwnldPage.equalsIgnoreCase("Yes")) {
					eForm.setDwnldChallanAmt(eForm.getParentAmount());
					logger.debug("inside challan download");
					String receivedFunId = eForm.getFuncId();

					serviceId = eForm.getServiceId();
					...
					if (receivedFunId.equalsIgnoreCase("FUN_236") && serviceId != null)
						revlist = paymentBD.getRevenueHeads(receivedFunId, serviceId);
					else
		...
		if (request.getParameter("OnlineDownload") != null) {
			...
			if (!(onlineDwnldPage).equalsIgnoreCase("")) {
				...
				if (onlineDwnldPage.equalsIgnoreCase("Yes")) {
					...
					String districtID = eForm.getParentDistId();
					logger.debug("districtID in PaymentAction----" + districtID);
					String officeId = eForm.getParentOffId();
					logger.debug("officeId in PaymentAction----" + officeId);
					if (districtID != null) {
		...
		if ("challanDownloadPage".equalsIgnoreCase(formName)) {
			...
			// Modified by gulrej gera
			// condition added for FUN_504
			// start of printToPDF action
			if (actionName.equalsIgnoreCase("printToPDF")) {
				try {
					if (!(eForm.getFuncId().equalsIgnoreCase("FUN_200") || eForm.getFuncId().equalsIgnoreCase("FUN_300") || eForm.getFuncId().equalsIgnoreCase("FUN_504")|| eForm.getFuncId().equalsIgnoreCase("FUN_600"))) {
						double paidAmt = 0.0;
						double payableAmt = Double.parseDouble(paymentBD.getPayableAmount(eForm));
						if (eForm.getParentAmount() != null) {
							eForm.setDwnldChallanAmt(eForm.getParentAmount());
							paidAmt = Double.parseDouble(eForm.getDwnldChallanAmt());
						}
					...
					// eForm.setParentRefId("1234");
					// if(eForm.getParentRefId().startsWith("SP")){
					...
					if (trans) {
						...
						if (mainlist.size() > 0) {
							...
							refrenceId = (String) sublist.get(1) != null ? (String) sublist.get(1) : "NA";
							System.out.println("here" + refrenceId);
							...
							// title1.setBackgroundColor(new Color(0xC0, 0xC0,
							// 0xC0));
							...
							// START | Challan generation date changes by
							// santosh
							Date date = new Date();
							DateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy");
							// System.out.println(dateFormat.format(date));

							Cell titl4 = new Cell(new Phrase("Challan Generation Date: " + dateFormat.format(date), FontFactory.getFont(FontFactory.COURIER, 8)));
							...
							// datatable.setBorderWidth(2);
							datatable2.setAlignment(1);
							// END | Challan generation date changes by santosh

							Cell titl5 = new Cell(new Phrase(" . ", FontFactory.getFont(FontFactory.COURIER, 8)));
							titl5.setHorizontalAlignment(Element.ALIGN_LEFT);
							// title1.setLeading(20);
							titl5.setColspan(5);
							titl5.setBorder(Rectangle.NO_BORDER);
							// title1.setBackgroundColor(new Color(0xC0, 0xC0,
							// 0xC0));
							datatable2.addCell(titl5);
							// datatable.setBorderWidth(2);
							datatable2.setAlignment(1);

							/*
							 * Cell title = new Cell(new
							 * Phrase("Vidya Bhawan, Bhopal",
							 * FontFactory.getFont(FontFactory.COURIER, 10,
							 * Font.BOLD)));
							 * title.setHorizontalAlignment(Element.ALIGN_LEFT);
							 * title.setLeading(20); title.setColspan(10);
							 * title.setBorder(Rectangle.NO_BORDER);
							 * //title.setBackgroundColor(new Color(0xC0, 0xC0,
							 * 0xC0)); datatable.addCell(title);
							 * //datatable.setBorderWidth(2);
							...
							// title2.setBackgroundColor(new Color(0xC0, 0xC0,
							// 0xC0));
							datatable.addCell(title2);
							datatable.setBorderWidth(1);
							datatable.setAlignment(1);

							// Cell title3 = new Cell(new
							// Phrase("CHALLAN No. "+(String)sublist.get(0),
							...
							// Cell title5 = new Cell(new
							// Phrase("at "+(String)sublist
							// .get(11)+", "+(String)
							// sublist.get(10)+" ("+(String)sublist.get(9)+")",
							// FontFactory.getFont(FontFactory.COURIER,
							// 10,Font.BOLD)));
							...
							// subTitle1.setBackgroundColor(new Color(0xC0,
							// 0xC0, 0xC0));
							...
							// subTitle2.setBackgroundColor(new Color(0xC0,
							// 0xC0, 0xC0));
							...
							// subTitle3.setBackgroundColor(new Color(0xC0,
							// 0xC0, 0xC0));
							...
							Cell subTitle3_1 = new Cell(new Phrase("Purpose - " + (String) sublist.get(3) + " Ref. No. - " + refrenceId, FontFactory.getFont(FontFactory.COURIER, 10, Font.BOLD)));
							...
							// subTitleBldg2.setBackgroundColor(new Color(0xC0,
							// 0xC0, 0xC0));
							...
							/*
							 * Cell blankrow4 = new Cell(new Phrase("",
							 * FontFactory.getFont(FontFactory.COURIER, 10,
							 * Font.BOLD)));
							 * blankrow4.setHorizontalAlignment(Element
							 * .ALIGN_CENTER); blankrow4.setLeading(20);
							 * blankrow4.setColspan(28);
							 * blankrow4.setBorder(Rectangle.NO_BORDER);
							 * //subTitleBldg2.setBackgroundColor(new
							 * Color(0xC0, 0xC0, 0xC0));
							 * datatable.addCell(blankrow4);
							 * datatable.setBorderWidth(2);
							 * datatable.setAlignment(1);
							 * 
							 * Cell blankrow5 = new Cell(new Phrase("",
							 * FontFactory.getFont(FontFactory.COURIER, 10,
							 * Font.BOLD)));
							 * blankrow5.setHorizontalAlignment(Element
							 * .ALIGN_CENTER); blankrow5.setLeading(20);
					...
					if (msg.equalsIgnoreCase("10002")) {
						...
						request.setAttribute("mesgBundle", PaymentConstant.ERROR_10002 + eForm.getSpActBal());
						forwardJsp = "processPaymentFailure";
						return mapping.findForward(forwardJsp);
					}
					if (msg.equalsIgnoreCase("5000")) {
						request.setAttribute("mesgBundle", PaymentConstant.ERROR_5000 + eForm.getParentRefId());
						forwardJsp = "invalidSPLicense";
						return mapping.findForward(forwardJsp);
					}
					// end of(trans)
				}
		...
		if ("onlineDownloadPage".equalsIgnoreCase(formName)) {
			...
			if (actionName.equalsIgnoreCase("districtToOffice")) {
				...
				eForm.setActionName("");
				eForm.setFormName("");
				jspPage = "onlineDwnldPage"; // added by roopam : 8 Oct 2013.
				// Bug id 930. resolved
			}// end of districtToOffice action
			...
			if (actionName.equalsIgnoreCase("forwardToTreasurySite")) {
				...
				try {
					...
					String majorhead="0030";
					if(eForm.getFuncId().equalsIgnoreCase("FUN_600")||eForm.getFuncId().equalsIgnoreCase("FUN_601"))
					{
						majorhead="0029";
					}
					String crn = bo.getCRNumber(majorhead);

					String urnNumber = bo.getURN();
					System.out.println("Ajit urnNumber --" + urnNumber);
					eForm.setUrn(urnNumber);
					// dto.setUrn(urnNumber);//Ajit URN
					logger.debug("Going to insert details in echallan and cyber offline tables");
					// get userid from session >> neeraj //Ajit IFMIS-1 Insert into IGRS_PAYMENT_ECHALLAN_DETAILS and IGRS_OFFLINE_CYBER table
					boolean trans = paymentBD.onlineDwnldInsert(eForm, userId, crn, jspPage);
					logger.debug("******************In side onlineDwnldInsert 1..." + trans);
					...
					if (mainlist.size() > 0) {
						...
						// eForm.setParentRefId(refrenceId);//
					...
					logger.debug("ipAddress from X-FORWARDED-FOR header --" + ipAddress);
					if (ipAddress == null) {
						ipAddress = request.getRemoteAddr();
						logger.debug("ipAddress from remote address --" + ipAddress);
					}
					...
					if (msg.equalsIgnoreCase("10002")) {
						eForm.setChallansearchRsptID("NoReceiptId");
						request.setAttribute("mesgBundle", PaymentConstant.ERROR_10002);
						forwardJsp = "processPaymentFailure";
						return mapping.findForward(forwardJsp);
					}
			...
			if (actionName.equalsIgnoreCase("forwardToTreasurySiteBF")) {
				...
				try {
					...
					// boolean trans = paymentBD.onlineDwnldInsert(eForm, userId,crn,jspPage);
					logger.debug("******************In side onlineDwnldInsert 2...");
					...
					logger.debug(" mapIpAddress :- " + mapIpAddress);
					String ipAddressLoggedIn = mapIpAddress.get(userId); // get
					// from
					// session
					// neeraj
					logger.debug(" ipAddressLoggedIn :- " + ipAddressLoggedIn);
					if (ipAddressLoggedIn == null) {
						throw new Exception("30005");
					}
					// Checking for IP address while logging in and at payment time.
					logger.debug("ip address from payment form ---" + eForm.getOnlineIp());
					if (eForm.getOnlineIp() != null && !ipAddressLoggedIn.equalsIgnoreCase(eForm.getOnlineIp())) {
						jspPage = "failure";
						throw new Exception();
					}
					logger.debug(" ipAddressLoggedIn Checked:- " + eForm.getCrnNumber() + ">>>>>>>>>>>>" + eForm.getOnlineIp());
					...
					// String url=PaymentConstant.TREASURY_URL_TEST_IFMIS;// Ajit IFMIS need to confirm it this code really required
					...
					System.out.println("Going to set challan number Ajit");
					dto.setChallanNumber(eForm.getChallanNumber());
					...
					ActionForm forms = (ActionForm) session.getAttribute(formNames); // remove
					// this
					// attribute
					// from
					// session
					...
					System.out.println("dto setting in map---" + dto.toString());
					LoadMenuExternal.getInstance().getDto().put(eForm.getCrnNumber(), dto); // Ajit putting Payment DTO
					// Added by Ajit for double verification Start
					boolean result = paymentBD.insertPaymentDTO(dto);
					// Added by Ajit for double verification End
					logger.debug("Before mp_treasury_key  ---" + result);
					...
					String keyPath = proper.getValue("mp_treasury_key_ifmis");
					...
					// added by validate CRN

					String treasuryArr[] = paymentBD.getTreasuryData(eForm.getCrnNumber());// Ajit IFMIS issue2
					logger.debug("TreasuryDataAction:-" + treasuryArr);
					String treasurydistId = paymentBD.getTreasuryDistrict(eForm.getParentDistId());
					String descriptionHOA = paymentBD.getDescriptionHOA(treasuryArr[0], treasuryArr[1], treasuryArr[2]);
					String user_id = session.getAttribute("UserId").toString();

					// Added by Siddhartha - Start
					String Ru = request.getRequestURL().toString();
					logger.debug("Ru from request ----" + Ru);

					String Lang = (String) session.getAttribute("languageLocale");
					String parent_treasury_code = null;
					String total_amount = null;
					String furl = Ru.substring(0, Ru.indexOf("IGRS/"));
					logger.debug("furl from request ----" + furl);
					furl = furl + "IGRS/";
					logger.debug("furl 2 from request ----" + furl);
					furl = furl + "onlinepymtrecpt.do";
					logger.debug("furl 3 from request ----" + furl);
					// Added by Ajit for production
					if (furl.contains("mpigr")) {
						furl = "https://www.mpigr.gov.in:8080/IGRS/onlinepymtrecpt.do";
						logger.debug("Hardcoded url for production on load balancer---" + furl);
					}
					SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");
					Date date = new Date();

					System.out.println("Making request url and urn is---" + eForm.getUrn());

					String functionName = paymentBD.getFunctionName(eForm.getFuncId(), "en");
					eForm.setFunName(functionName);
					System.out.println("Purpose of payment----" + eForm.getFunName());
					String dept_code = "05";
					// Ajit IFMIS-1 encrypting all payment related form parameters / total 36 parameters to treasury
					String[] a = new String[]{"crn", "ru", "fru", "name", "add_line1", "add_line2", "add_line3", "add_line4", "add_line5", "add_state", "add_pin", "email", "mob_no", "tin", "from_date", "to_date", "major_head", "submajor_head", "minor_head", "scheme_code", "purpose", "amount", "total_amount", "remarks", "circle_code", "deptt_code", "dist_code", "office_name", "IFSC", "account_number", "account_name", "New1", "New2", "New3", "urn"};
					String[] b = new String[]{eForm.getCrnNumber(), furl, furl, treasuryArr[5].trim(), eForm.getIgrsadd1(), eForm.getIgrsadd2(), eForm.getIgrsadd3(), eForm.getIgrsadd4(), eForm.getIgrsadd5(), eForm.getState(), eForm.getPin(), eForm.getEmailAddress(), eForm.getMobile(), "", formatter.format(date), formatter.format(date), treasuryArr[0].trim(), treasuryArr[1].trim(), treasuryArr[2].trim(), treasuryArr[3].trim(), eForm.getFunName(), treasuryArr[6].trim(), treasuryArr[6].trim(), "", "00", dept_code, treasurydistId, treasuryArr[8].trim(), "", "", "", "", "", "", eForm.getUrn()};
					// String []b = new String[]{"MPGOVT_IGR",urnNumber,eForm.getCrnNumber(),furl,furl,treasuryArr[5].trim(),eForm.getIgrsadd1(),eForm.getIgrsadd2(),eForm.getIgrsadd3(),eForm.getIgrsadd4(),eForm.getIgrsadd5(),eForm.getState(),eForm.getPin(),eForm.getEmailAddress(),eForm.getMobile(),formatter.format(date),formatter.format(date),treasuryArr[0].trim(),treasuryArr[1].trim(),treasuryArr[2].trim(),treasuryArr[3].trim(),eForm.getFunName(),treasuryArr[6].trim(),treasuryArr[6].trim(),"00","30",treasurydistId,eForm.getParentOffName(),eForm.getTin(),eForm.getRemarks(),eForm.getIfsc(),eForm.getAccount_number(),eForm.getAccount_name(),eForm.getNew1(),eForm.getNew2(),eForm.getNew3()};

					String urls = paymentBD.createUrl(a, b, ""); // Ajit IFMIS-2 // getting clubbed string
					// changes on 18 dec 2015 by Shreeraj
					logger.debug("CRN number is---" + eForm.getCrnNumber());
					paymentBD.insertURL(eForm.getCrnNumber(), "O", urls); // Ajit IFMIS-3 inserting payment URL into IGRS_OFFLINE_CYBER table against CRN number
					// changes on 22 Jan 2015 by Shreeraj
					boolean validateCRN = paymentBD.validateCRN(userID, eForm); // Ajit IFMIS-4 validating CRN from IGRS_PAYMENT_ECHALLAN_DETAILS table
					if (validateCRN == false) {
					...
					String encoded = enc.encryptFile(urls); // Ajit IFMIS-5 encrypting payment input string as per IGRS_KEY.key
					...
					logger.debug("Before SecretKey encodedUrl..." + encodedUrl);
					logger.debug("Before SecretKey decode...");

					PropertiesFileReader property = PropertiesFileReader.getInstance("resources.igrs");
					String onlineURL = property.getValue("online_url_final_ifmis");
					String finalEncodedUrl = onlineURL + "?ctp_data=" + encodedUrl + "&merchant_code=IGRS";// Ajit IFMIS-6 final payment url that will be called from JSP onlineDownload2Session.jsp , POST method
					// String ctp_data=encodedUrl+"&merchant_code=IGRS";
					...
					eForm.setTreasuryUrl(finalEncodedUrl); // Ajit IFMIS-7 added payment URL to call from JSP onlineDownload2Session.jsp
					session.setAttribute("treasuryurl", finalEncodedUrl); // Ajit Final treasury url
					logger.debug("After eForm.setTreasuryUrl...");
					// Added by Ajit For IE browser issue
					session.setAttribute("ifmisurl", onlineURL);
					session.setAttribute("ctp_data", encoded);
					logger.debug("Before onlineDwnldPage2...");
					jspPage = "onlineDwnldPage2"; // added by roopam : 8 Oct 2013. Bug id 930. resolved
					logger.debug("Before onlineDwnldPage2...");

				} catch (Exception ex) {
					// logger.debug("After clicking on Cash radio button"+ex.
					// getMessage());

					logger.debug("");
					String msg = ex.getMessage();
					logger.error("After clicking on Cash radio button 2" + ex.getMessage());
					if (msg.equalsIgnoreCase("10002")) {
						eForm.setChallansearchRsptID("NoReceiptId");
						request.setAttribute("mesgBundle", PaymentConstant.ERROR_10002);
						forwardJsp = "SPSwapPay";
						return mapping.findForward(forwardJsp);
					} else if (msg.equalsIgnoreCase("30005")) {
						eForm.setChallansearchRsptID("NoReceiptId");
						request.setAttribute("mesgBundle", PaymentConstant.ERROR_30005);
						forwardJsp = "ipfailure";
						return mapping.findForward(forwardJsp);
					}
					// ex.printStackTrace();
			...
			/*
			 * formName=""; actionName=""; eForm.setActionName("");
			 * eForm.setFormName(""); jspPage="onlineDwnldPage2";
			 */// commented by roopam : 8 Oct 2013. Bug id 930. resolved
		} // end of onlineDownloadPage

		// ------------Start for SRO Page-------------//
		if (PaymentConstant.SRO_PAGE.equals(formName)) {
			logger.debug("After clicking on Cash radio button 1");
			try {
				if (PaymentConstant.SEARCH_RECEIPT_ACTION.equals(actionName)) {
					...
					try {
						 ...
						 * eForm.setSearchRsptID("NoReceiptId");
						 * request.setAttribute("mesgBundle",
						 * PaymentConstant.ERROR_30004); forwardJsp = jspPage; }
						 */
				...
				if (PaymentConstant.SEARCH_CHALLAN_RECEIPT_ACTION.equals(actionName)) {
					...
					// String bankid = eForm.getChallanBankId(); // commentd to
					// remove bankid
					logger.debug("IN ACTION........AMOUNT ENTERED BY USER....." + entAmt);
				...
				if (PaymentConstant.NEXT_ACTION.equals(actionName)) {
					...
					logger.debug("ON CLICK OF PAYMENT BUTTON...Checking for consumed id 1");
					...
					true || dtoPayment.getSelectType().equalsIgnoreCase("online")) {
						logger.debug("ON CLICK OF PAYMENT BUTTON...Checking for consumed id 1 1");
						...
						if ("cash".equals(selectType)) {
							try {
								...
								boolean trans = paymentBD.PaymentTransactionFinal(eForm);
								if (trans) {
									String transid = eForm.getTransId();
									request.setAttribute("PayTransId", transid);
									request.setAttribute("PaidAmt", entAmt);
						...
						if ("challan".equals(selectType)) {
							try {
								...
								// method for getting dist id and office id from
								// challan id.
						...
						if ("online".equals(selectType)) {
							try {
								...
								if (msg.equalsIgnoreCase("40000")) {
									...
									eForm.setOnlineSearchRsptID("NoReceiptId");
									request.setAttribute("mesgBundle", PaymentConstant.ERROR_10025);
									forwardJsp = "SPSwapPay";
									return mapping.findForward(forwardJsp);
						...
						/*
						 * if("challan".equals(selectType)) {
						 * eForm.setFuncId(funid); eForm.setLoggedUser(userId);
						 * logger
						 * .debug("eForm.getPaymentList():-"+eForm.getPaymentList
						 * ()); if(eForm.getPaymentList()!=null) {
						 * 
						 * for (int i = 0; i < eForm.getPaymentList().size();
						 * i++) { eForm.setPayMode("IGRS_PT_002"); getStatus =
						 * paymentBD.getChallanDetails(eForm, amount);
						 * eForm.getPaymentDTO().setSelectType("");
						 * session.setAttribute("PayMode", "IGRS_PT_002"); if
						 * ("fail".equalsIgnoreCase(getStatus)) {
						 * session.setAttribute("status", getStatus);
						 * logger.debug("inside Challan Payment"); //return
						 * mapping.findForward("checkPayment"); forwardJsp =
						 * "checkPayment"; } else {
						 * session.setAttribute("status", getStatus); double
						 * totamt = eForm.getTotAmt(); String finalamt =
						 * String.valueOf(totamt);
						 * session.setAttribute("totamt",
						 * String.valueOf(totalAmt)); PaymentDTO paymentDto =
						 * new PaymentDTO(); paymentDto = null;
						 * eForm.setPaymentDTO(paymentDto);
						 * logger.debug("inside Challan Process Payment");
						 * forwardJsp = "processPayment"; //return
						 * mapping.findForward("processPayment"); } } } }
						 */
						// resetToken(request); //ramesh commented on 14 dec 12
						eForm.setActionName(""); // ramesh commented on 14 dec
						// 12
						// eForm.setFormName(""); //ramesh commented on 14 dec
						// 12
					}
				}
				...
				if (("challancheck".equals(actionName))) {
					...
					getStatus = paymentBD.getChallanNoDetails(eForm, i);
					// session.setAttribute("status",getStatus);
					forwardJsp = jspPage;
					// forwardJsp = "SP";
					// saveToken(request);//CSRF
		 ...
		 * 
		 * if
		 * ("spProcess".equalsIgnoreCase(eForm.getPaymentDTO().getSpProcess()))
		 * // if ("spProcess".equals(actionName)) {
		 * eForm.getPaymentDTO().setSpProcess(""); PaymentDTO paymentDTO = new
		 * PaymentDTO(); paymentDTO = eForm.getPaymentDTO(); String spTxnId =
		 * paymentBD.setSpDetails(paymentDTO);
		 * 
		 * session.setAttribute("status", spTxnId); session.setAttribute(
		 * "totamt", (String) session .getAttribute("amount")); PaymentDTO
		 * paymentDto = new PaymentDTO(); paymentDto = null;
		 * eForm.setPaymentDTO(paymentDto); logger.debug("inside SPProcess");
		...
		if ("SPSearchPage1".equalsIgnoreCase(formName)) {
			...
			try {
				...
				if (PaymentConstant.NEXT_ACTION.equals(actionName)) {
					...
					if (
					// isTokenValid(request) ||//CSRF
					true || "online".equalsIgnoreCase(selectType)) {
						logger.debug("ON CLICK OF PAYMENT BUTTON-SP1 Page");
						...
						if ("challan".equals(selectType)) {
							try {
								...
								if (funId.equalsIgnoreCase("FUN_504")) {
									challantrans = paymentBD.JudicialChallanPayment(eForm, session);
								}

								if (funId.equalsIgnoreCase("FUN_600")) {
									challantrans = paymentBD.MutationChallanPayment(eForm, session);
								}
								if (challantrans) {
									String transid = eForm.getTransId();
									request.setAttribute("PayTransId", transid);
						...
						if ("online".equals(selectType)) {
							try {
								...
								if (funId.equalsIgnoreCase("FUN_504")) {
									challantrans = paymentBD.JudicialPaymentOnlineTransactionFinalSP(eForm, session);
								}
								if (funId.equalsIgnoreCase("FUN_601")) {
									logger.debug("Function ID is --> "+funId);
									challantrans = paymentBD.PaymentOnlineTransactionFinalSP3(eForm, session);
								}
				...
				/*
				 * if ("selectedCredit".equals(actionName)) {
				 * eForm.setRadioSelect("credit"); forwardJsp = jspPage;
				 * saveToken(request); }
				 */
		...
		if ("SPSearchPage2".equalsIgnoreCase(formName))// Ajit
		{
			try {
				if (PaymentConstant.SEARCH_CHALLAN_RECEIPT_ACTION.equals(actionName)) {
				...
				if (PaymentConstant.NEXT_ACTION.equals(actionName)) {
					logger.debug("ON CLICK OF PAYMENT BUTTON...Checking for consumed id 2");
					PaymentDTO dtoPayment = eForm.getPaymentDTO();
					String selectType = dtoPayment.getSelectType();
					if (!dtoPayment.getSelectType().equalsIgnoreCase("online")) {
						// if(!isTokenValid(request)) {//CSRF
					...
					// isTokenValid(request) || //CSRF
					true || dtoPayment.getSelectType().equalsIgnoreCase("online")) {
						logger.debug("ON CLICK OF PAYMENT BUTTON...Checking for consumed id 2 2");
						...
						if ("credit".equals(selectType)) {
							try {
								...
								logger.debug("amount balacne-------" + b);
								eForm.setPayMode("4");
								eForm.setFuncId(funid);
								logger.debug("funid id -------" + funid);
								eForm.setLoggedUser(userId);
								ServiceProviderAccountBD spbd = new ServiceProviderAccountBD();
								...
								logger.debug("OUTSIDE CREDIT VALIDATION-------" + comsn);
								eForm.setSpComsn(comsn1);
								eForm.setSpAmt(spamt);
								eForm.setSpamtd(spamt1);
								String spbalanceString = eForm.getPaymentDTO().getSpAmt();
								// first = paymentBD.getPayDtls(eForm);
								double payableAmt = Double.parseDouble(paymentBD.getPayableAmount(eForm));
								...
								// Changes by preeti on 16 dec 2015
								String sessionLicenseNo = "";
								sessionLicenseNo = (String) session.getAttribute("licNo");
								logger.debug("Session License Number at the of creditTrabs update" + sessionLicenseNo);
								// end os changes
						...
						/*
						 * if("credit".equals(selectType)){
						 * logger.debug("INSIDE ACTION FOR SP"); String a =
						 * eForm.getCrEntrAmt(); double b =
						 * Double.parseDouble(a);
						 * eForm.setPayMode("IGRS_PT_004");
						 * eForm.setFuncId(funid); PaymentDAO paymentDao = new
						 * PaymentDAO(); //getStatus1 =
						 * paymentBD.getCreditDetails(eForm, b); getStatus =
						 * paymentBD.getCreditDetails1(eForm, userId, b);
						 ...
						 * 
						 * if (getStatus.equalsIgnoreCase("fail")) { jspPage =
						 * "checkPayment"; } else { ServiceProviderAccountBD
						 * spbd = new ServiceProviderAccountBD(); boolean dbt =
						 * false; dbt = spbd.getSPBalanceAfterDebit(userId,
						 * getStatus, "IGRS_PT_004", a); if (dbt == true){
						 * session.setAttribute("status", getStatus);
						 * //session.setAttribute("PaidAmt", a);
						 * session.setAttribute("PayMode", "IGRS_PT_004");
						 * jspPage = "processPayment";
						 * logger.debug("INSIDE CREDIT....VALUE OF JSP"
						 * +jspPage); }else{jspPage = "checkPayment";} }
						 * forwardJsp = jspPage; }
						 */
				...
				if ("selectedCredit".equals(actionName)) {
					System.out.println("selectedCredit block :: " + actionName);
					logger.debug("selectedCredit block ::  " + actionName);
					eForm.setRadioSelect("credit");
					eForm.setCrEntrAmt(parentAmount);
					eForm.setParentAmount(parentAmount);
					eForm.setCheckAvlblty(0);
					...
					try {
						...
						ArrayList spListInner = new ArrayList();

						// preeti changes

						String sessionLicenseNo = "";
						sessionLicenseNo = (String) session.getAttribute("licNo");
						spList = paymentBD.getSpDetails(userId, funid, sessionLicenseNo);

						System.out.println("Size of list" + spList.size());

						String licenseNo = "";

						spListInner = (ArrayList) spList.get(0);
						for (int j = 0; j < spListInner.size(); j++) {

							licenseNo = spListInner.get(0).toString();

						}
						double paymentAmt = 0.0;

						// Case 1:mismatch in License no
						if (sessionLicenseNo.equalsIgnoreCase(licenseNo)) {
							boolean balanceStatus = false;
							balanceStatus = paymentBD.getAcctBalance(sessionLicenseNo, funid);

							if (balanceStatus != false) {
								double spAcctBal = Double.parseDouble(paymentBD.getAcctBal(sessionLicenseNo, funid));

								// Added By Gulrej Gera - Start
								if (eForm.getFuncId().trim().equals("FUN_022")) {
									String transactionId = eForm.getParentRefId();
									List paymentDtls = new ArrayList();
									List comp = new ArrayList();
									String amtToBePaid = null;
									paymentDtls = objEstampBO.getPayDtls(eForm.getParentRefId().toString());
									if (paymentDtls.size() > 0) {
										for (int i = 0; i < paymentDtls.size(); i++) {
											comp.add((ArrayList) paymentDtls.get(i));
									...
									// double paymentAmt =
									// Double.parseDouble(eForm.getCrEntrAmt());
									paymentAmt = Double.parseDouble(amtToBePaid);
									eForm.setCrEntrAmt(amtToBePaid);
								} else {
									paymentAmt = Double.parseDouble(eForm.getCrEntrAmt());
									logger.debug("paymentAmt number  " + paymentAmt);
								}

								// Added By Gulrej Gera - End

								if (spList.size() > 0) {
									if (spAcctBal >= paymentAmt) {

										logger.debug("balance Of SP at time of finding acctount Balance  " + spAcctBal);
										logger.debug("Paid amount Of SP " + paymentAmt);
										for (int i = 0; i < spList.size(); i++) {
											// SELECT LICENSE_NO, LICENSE_TXN_ID
											// FROM IGRS_SP_USER_LICENSE_DETAILS
											ArrayList list = (ArrayList) spList.get(i);
											eForm.getPaymentDTO().setSpLicenseNo((String) list.get(0));
											eForm.setSpLicenceNo((String) list.get(0));
											eForm.getPaymentDTO().setSpAmt((String) list.get(1));
											// eForm.getPaymentDTO().setSpAmt(
											// Double.toString(spAcctBal));
											eForm.getPaymentDTO().setSpName((String) list.get(2));
											eForm.getPaymentDTO().setUser((String) list.get(3));
											String sessionUserId = (String) session.getAttribute("UserId");
											if (!sessionUserId.equalsIgnoreCase(eForm.getPaymentDTO().getUser())) {
												forwardJsp = "SPSwapPay";
											} else {
												forwardJsp = "SP";
											}
											logger.debug("forwared page in case of suucess" + forwardJsp);
											// saveToken(request);//CSRF
										}
									} else {

										logger.debug("balance  " + spAcctBal);
										forwardJsp = "SPAcctBalFail";
										logger.debug("forwared page in case of fail" + forwardJsp);
									}
								}
							...
							forwardJsp = "SPCreditLimitFail";
							logger.debug("forwared page in case of sp mismatch" + forwardJsp);

						}

						// preeti changes close
		...
		if ("PaymentFailPage".equals(formName)) {
...

			{
				 ...
				 * eForm.getPaymentDTO().setSelectButton(""); String failflag =
				 * "fail"; session.setAttribute("failflag", failflag); failflag
				 * = null; challanList.clear();
				 * session.setAttribute("challanList", challanList);
