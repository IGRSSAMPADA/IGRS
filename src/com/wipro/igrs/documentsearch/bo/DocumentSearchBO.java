...
public class DocumentSearchBO {
	...
	/*
	 * public Map getPartyAllData(String regNumber) throws Exception { return dsdao.getPartyAllData(regNumber, new ArrayList()); }
	 */

	// added by saurav
	public ArrayList getTehsil(String distId, String lang) throws Exception {
		ArrayList result = null;
		result = dsdao.getTehsil(distId, lang);
		return result;
	}
	// added by saurav
	public ArrayList getTehsilGDL(String distId, String lang, String fisYear) throws Exception {
		ArrayList result = null;
		result = dsdao.getTehsilGDL(distId, lang, fisYear);
		return result;
	}
	public ArrayList<PropertyValuationDTO> getSubArea(String language, String areaId, String tehsilId, String urbanFlag, String fisYear) throws Exception {
		...
		try {
			...
			subAreaList = dsdao.getSubArea(language, areaId, tehsilId, urbanFlag,fisYear);
			if (subAreaList != null && subAreaList.size() > 0) {
				...
				for (int i = 0; i < subAreaList.size(); i++) {
					...
					if ("en".equalsIgnoreCase(language)) {
						...
						propDTO.setSubAreaName((String) subList.get(2));
					}
	...
	public ArrayList<PropertyValuationDTO> getWardPatwari(String language, String subAreaId, String tehsilId, String fisYear) {
		ArrayList wardPatwariList = null;
		ArrayList<PropertyValuationDTO> returnList = null;
		try {
			wardPatwariList = dsdao.getWardPatwari(language, subAreaId, tehsilId, fisYear);
			if (wardPatwariList != null && wardPatwariList.size() > 0) {
				...
				for (int i = 0; i < wardPatwariList.size(); i++) {
					...
					PropertyValuationDTO propDTO = new PropertyValuationDTO();
					propDTO.setWardPatwariId((String) subList.get(0));
					if ("en".equalsIgnoreCase(language)) {
	...
	public ArrayList<PropertyValuationDTO> getColonyName(String language, String colonyId, String fisYear) {
		ArrayList l1NameList = new ArrayList();
		ArrayList<PropertyValuationDTO> returnList = null;
		try {
			l1NameList = dsdao.getColonyName(language, colonyId, fisYear);
			if (l1NameList != null && l1NameList.size() > 0) {
				...
				for (int i = 0; i < l1NameList.size(); i++) {
					...
					if ("en".equalsIgnoreCase(language)) {
						...
						propDTO.setColonyName((String) subList.get(2));
					}
					returnList.add(propDTO);
	...
	public ArrayList checkRegistrationIdOffice(String _refRegId, String language) throws Exception {
		...
		//	resultList = dsdao.checkRegistrationId(_refRegId, language);
			resultList = dsdao.checkRegistrationIdOffice(_refRegId, language);
			// logger.info("in bo checkRegistrationId-->"+resultList);
	...
	public String getFiscalYearName(String fiscalYearId) throws Exception{
		String retval="";
		try{
			retval=dsdao.getFiscalYearName(fiscalYearId);
		}catch (Exception ex){
			if(fiscalYearId==null){
				logger.debug("fiscal year has not been selected");
			}else{
				logger.debug("Some error occured while fetching fiscal year name");
			}
			ex.printStackTrace();
		}
		return retval;
