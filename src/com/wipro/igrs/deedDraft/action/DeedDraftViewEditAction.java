...
import com.wipro.igrs.deedDraft.bd.DeedDraftViewEditBD;
...
public class DeedDraftViewEditAction extends BaseAction {
	String forwardJsp = "";
	private Logger logger = Logger.getLogger(DeedDraftViewEditAction.class);
	...
	@Override public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		...
		if (form != null) {
			...
			DeedDraftViewEditBD bd = new DeedDraftViewEditBD();
			...
			if (label != null) {
				if (label.equalsIgnoreCase("view")) {
					...
					formDTO.setUpOrNot("");
					formDTO.setErrorFlag("");
					formDTO.setActionName("");
					formDTO.setLastPageChk("");
					formDTO.setFirstPageChk("");
					formDTO.setEditCheck("");
					formDTO.setEditFlag("");
					formDTO.setAddMorePage("");
					//saurav
					...
					formDTO.setDeedCheck("");
					formDTO.setDeedAppStatus("");
					...
					forwardJsp = "viewEditDashboard";
				} else if (label.equalsIgnoreCase("deleteDeed")) {
					eForm.setDeedIdList(new ArrayList());
					ArrayList deedViewDetails = bd.getDraftDeedDeleteDetailsForDashboard(userId, language);
					eForm.setDeedIdList(deedViewDetails);
					if (deedViewDetails.size() > 0) {
						eForm.setDeedDraftViewList(deedViewDetails);
						request.setAttribute("deedDraftdeleteList", deedViewDetails);
					} else {
						if (language.equalsIgnoreCase("en"))
							request.setAttribute(DeedDraftConstant.FAILURE_MSG, "No records found");
						else
					...
					formDTO.setFirstPageLimit(pr.getValue("deed_draft_first_page_limit"));
					formDTO.setRestPagesLimit(pr.getValue("deed_draft_rest_pages_limit"));
					forwardJsp = "viewDelete";
				}
			}
			if (request.getParameter("deedId") != null /*&& formDTO.getActionName()=="DELETE_STATE"*/) {
				if (DeedDraftConstant.delete_deed.equalsIgnoreCase(formDTO.getActionName()))
					formDTO.setActionName(DeedDraftConstant.delete_deed);
				else
					//formDTO.setActionName(DeedDraftConstant.VIEW_DETAILS);
					formDTO.setActionName("appTypeCheck");
			}
			/*else{
				formDTO.setActionName(DeedDraftConstant.delete_deed);
			...
			if (("appTypeCheck").equals(actionName)) {
				String deedAppId = eForm.getDeedId();
				String regTxnId = bd.getRegTxnId(deedAppId);
				String appType = bd.getAppType(deedAppId);
				String masterCheck = bd.getMasterCheck(deedAppId);
				String deedStatus = bd.getDeedStatus(deedAppId);
				formDTO.setDeedAppStatus(deedStatus);
				if (!("Y".equals(masterCheck))) {
					if (!("".equals(regTxnId))) {
						formDTO.setRegTxnId(regTxnId);
						formDTO.setActionName("appValidate");
						actionName = "appValidate";
						formDTO.setDeedCheck("Y");
					} else if ("E".equals(appType)) {
						eForm.setAppTypeSelect("1");
						formDTO.setActionName("appCheck");
						actionName = "appCheck";
					} else if ("D".equals(deedStatus)) {
						eForm.setIsValidate("1");
						eForm.setIsProp("");
						eForm.setIsReg("");
						eForm.setIsSelected("Y");
						eForm.setAppTypeCheck("N");
						eForm.setIsEstampExemp("");
						eForm.setIsRegExemp("");
						...
						String maxPage = bd.getMaxPageNumber(deedId);
						if (maxPage.equals(formDTO.getPageId())) {
							formDTO.setLastPageChk("Y");
						}
						formDTO.setDeedId(deedId);
						formDTO = bd.getDeedContent(deedId, formDTO.getPageId(), formDTO);
						formDTO.setEditFlag("N");
						forwardJsp = "viewEdit";
					} else {
						deedAppDTO.setPropType("");
						deedAppDTO.setDutyTxnId("");
						...
						eForm.setAppTypeCheck("Y");//for selecting creation of deed of which process
			...
			if ("appValidate".equals(actionName)) {
				String isVal = "N";
				String propCheck = "N";
				...
				if (list != null && !list.isEmpty()) {
					if (list.get(0).equals(regTxnId)) {
						if (list.get(2).equals(userId)) {
							...
							if (!(list.get(1) == null) && !("Y".equals(formDTO.getDeedCheck()))) {
								if ("en".equals(language)) {
									eForm.setErrorMessage("Deed draft has already been created for " + regTxnId + " which is " + list.get(1));
								} else {
									eForm.setErrorMessage(regTxnId + " के लिए डीड ड्राफ्ट पहले से ही बनाया गया है जो " + list.get(1) + " है|");
								}
								eForm.setIsValidate("0");
								eForm.setAppTypeCheck("");
								formDTO.setEditFlag("N");
							} else if (!(("5").equals(list.get(5)) || ("3").equals(list.get(5)) || ("N").equals(propCheck) || ("6").equals(list.get(5)) || ("D").equals(formDTO.getDeedAppStatus()))) {
								eForm.setIsReg("");
								eForm.setIsValidate("0");
								eForm.setAppTypeCheck("");
								eForm.setIsProp("");
								...
								ArrayList propDutyDetailsTemp = new ArrayList();
								...
								if (!("N").equals(propCheck)) {
									...
									for (int k = 0; k < propDetails.size(); k++) {
										...
										if (propDutyDetailsTemp != null && !propDutyDetailsTemp.isEmpty()) {
											...
											ArrayList userEnterableValue = bd.getUserEnterableValue(dutyTxnId, (String) propDutyDetails.get(2), language);
										...
										if ("1".equals(propertyType)) {
											...
											for (int i = 0; i < agriPropDetails.size(); i++) {
												...
												if ("Y".equals(d1.getCommonAgriConstruction())) {
													...
													//construction slab rate changes by saurav
													...
													//end of construction slab rate changes 
													if ("4".equals(buildingId)) {
			...
			} else if ("appCheck".equals(actionName)) {
			...
			} else if (DeedDraftConstant.NEXT_DETAILS.equalsIgnoreCase(actionName)) {
				int pageId = Integer.parseInt(formDTO.getPageId());
				formDTO.setPageId(String.valueOf((pageId + 1)));
				formDTO = bd.getDeedContent(formDTO.getDeedId(), formDTO.getPageId(), formDTO);
				formDTO.setEditFlag("N");
				String maxPage = bd.getMaxPageNumber(formDTO.getDeedId());
				if (maxPage.equals(formDTO.getPageId()))
					formDTO.setLastPageChk("Y");
				if (!formDTO.getPageId().equals("1"))
					formDTO.setFirstPageChk("N");
				forwardJsp = "viewEdit";
			} else if (DeedDraftConstant.BACK_ACTION.equalsIgnoreCase(actionName)) {
				int pageID = Integer.parseInt(formDTO.getPageId());
				formDTO.setEditFlag("N");
				formDTO.setLastPageChk("N");
				formDTO.setPageId(String.valueOf(pageID - 1));
				String maxPage = bd.getMaxPageNumber(formDTO.getDeedId());
				if (maxPage.equals(formDTO.getPageId()))
					formDTO.setLastPageChk("Y");
				...
				formDTO = bd.getDeedContent(formDTO.getDeedId(), formDTO.getPageId(), formDTO);
				forwardJsp = "viewEdit";
			} else if (DeedDraftConstant.EDIT_ACTION.equalsIgnoreCase(actionName)) {
				bd.changeStatus(formDTO.getDeedId());
				formDTO.setEditFlag("Y");
				forwardJsp = "viewEdit";
			} else if (DeedDraftConstant.SAVE_EDIT_ACTION.equalsIgnoreCase(actionName)) {
				String appType = "E";
				...
				if (flag) {
					if (language.equalsIgnoreCase("en"))
						request.setAttribute(DeedDraftConstant.SUCCESS_MSG, "Data has been saved successfully.");
					else
						request.setAttribute(DeedDraftConstant.SUCCESS_MSG, "डेटा को सफलतापूर्वक सुरक्षित किया गया है |");
					forwardJsp = "viewEdit";
				} else {
					if (language.equalsIgnoreCase("en"))
			...
			} else if (DeedDraftConstant.SAVE_NEXT_EDIT_ACTION.equalsIgnoreCase(actionName)) {
				String appType = "E";
				if ("1".equals(eForm.getAppTypeSelect())) {
					appType = "E";
				} else {
				...
				if (maxPage.equals(formDTO.getPageId()))
					formDTO.setLastPageChk("Y");
				if (formDTO.getAddMorePage() != null && formDTO.getAddMorePage().equalsIgnoreCase("Y"))
					formDTO.setEditFlag("Y");
				else
					formDTO.setEditFlag("N");
			} else if (DeedDraftConstant.FINISH_EDIT_ACTION.equalsIgnoreCase(actionName)) {
				String appType = "E";
				if ("1".equals(eForm.getAppTypeSelect())) {
					appType = "E";
				} else {
			...
			else if("Final_upload".equalsIgnoreCase(actionName))
			{
				boolean flag = bd.saveDeedDraftDetails(formDTO, userId);
			...
			else if (DeedDraftConstant.BACK_EDIT_ACTION.equalsIgnoreCase(actionName)) {
				...
				formDTO = bd.getDeedContent(formDTO.getDeedId(), formDTO.getPageId(), formDTO);
				forwardJsp = "viewEdit";
			} else if (DeedDraftConstant.ADD_MORE_PAGE_ACTION.equalsIgnoreCase(actionName)) {
				bd.changeStatus(formDTO.getDeedId());
				int pageID = Integer.parseInt(formDTO.getPageId());
				formDTO.setFirstPageChk("N");
				formDTO.setPageId(String.valueOf(pageID + 1));
				...
				formDTO.setEditFlag("Y");
				formDTO.setDeedContent("");
				formDTO.setAddMorePage("Y");
				forwardJsp = "viewEdit";
			} else if (DeedDraftConstant.ADD_MORE_PAGE_EDIT_ACTION.equalsIgnoreCase(actionName)) {
				String appType = "E";
				if ("1".equals(eForm.getAppTypeSelect())) {
					appType = "E";
				} else {
				...
				formDTO.setAddMorePage("Y");
				forwardJsp = "viewEdit";
			} else if (DeedDraftConstant.delete_deed.equalsIgnoreCase(actionName)) {
				String deedId[] = eForm.getValueCheckBox().split(",");
				boolean flag = false;
				eForm.setValueCheckBox("");
				logger.debug("sucess");
				formDTO.setActionName("");
				flag = bd.deleteDeed(deedId);
				eForm.setDeedIdList(new ArrayList());
				ArrayList deedViewDetails = bd.getDraftDeedDeleteDetailsForDashboard(userId, language);
				eForm.setDeedIdList(deedViewDetails);
				if (deedViewDetails.size() > 0) {
				...
				if (flag) {
					formDTO.setErrorFlag("DN");
				} else {
					formDTO.setErrorFlag("DY");
				}
				formDTO.setActionName(null);
				forwardJsp = "viewDelete";
			}
		}
