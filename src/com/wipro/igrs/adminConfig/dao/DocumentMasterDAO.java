 ...
 * Copyright   2007-08
 *==============================================================================
 * File Name	:	DocumentMasterDAO.java
 * Author		:	vengamamba
 * Date			: 	18/06/2008
 */
...
public class DocumentMasterDAO {
	...
	private Logger logger = (Logger) Logger.getLogger(DocumentMasterDAO.class);
	...
	/**
	 * Method : addDocumentMaster() Description : returns boolean gives success
	 * of insert or not
	 * @param userId 
	 * @param funId 
	 * @param roleId 
	...
	public boolean addDocumentMaster(String[] param, String roleId, String funId, String userId) throws Exception {
		...
		try {
			...
			query = CommonSQL.DOCUMENT_TYPE_MASTER_SAVE_DATA;
			...
			{
			dbUtil.commit();
			igrsCommon.saveLogDet("IGRS_DOCUMENT_TYPE_MASTER","INSERT","T",funId,userId,roleId);
			}

			else
			{
			dbUtil.rollback();
			igrsCommon.saveLogDet("IGRS_DOCUMENT_TYPE_MASTER","INSERT","F",funId,userId,roleId);
			}
		} catch (Exception x) {
	...
	/**
	 * Method : updateDocumentmaster Description : returns boolean gives success
	 * of UPDATE or not
	 * @param userId 
	 * @param funId 
	 * @param roleId 
	...
	public boolean updateDocumentmaster(String p[], String roleId, String funId, String userId) throws Exception {
		...
		try {
			...
			query = CommonSQL.DOCUMENT_TYPE_MASTER_UPDATE_DATA;
			dbUtil.createPreparedStatement(query);
			logger.debug("the query is  in DAO   " + query);
			...
			{
			dbUtil.commit();
			igrsCommon.saveLogDet("IGRS_CONFIG_PARAM_LIST","UPDATE","T",funId,userId,roleId);
			}
			else
			{
			dbUtil.rollback();
			igrsCommon.saveLogDet("IGRS_CONFIG_PARAM_LIST","UPDATE","F",funId,userId,roleId);
			}
		} catch (Exception x) {
	...
	/**
	 * Method : getDocumentList Description : returns total data of
	 * partytypeMaster
	 * 
	 * @param query :
	...
	public ArrayList getDocumentList() throws Exception {
		...
		try {
			dbUtil = new DBUtility();
			query = CommonSQL.MASTER_DOCUMENT_TYPE_DATA;
			logger.debug("the query is  in DAO   " + query);
	...
	/**
	 * Method : getDocumentIdList() Description : returns data corresponding to
	 * id
	 * 
	 * @param query
	 *            :Documentid return Type :dto object
	 */
	public ArrayList  getDocumentIdList(String id)
			throws Exception {

		try {
			dbUtil = new DBUtility();
			query = CommonSQL.MASTER_DOCUMENT_TYPE_ID_DATA;
			logger.debug("the query is  in DAO   " + query);
			dbUtil.createPreparedStatement(query);
			list = new ArrayList();
