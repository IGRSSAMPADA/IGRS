...
if(fromParam.equalsIgnoreCase("uploadAdditional") )
{
	 ...
	 try {
			...
			//code added by preeti on 12 august 2016
			String fileName=regForm.getFileAUName()+"."+fileExt;
			
			
			boolean checkfileName=false;
			checkfileName=commonBo.getcheckfileName(regForm.getHidnRegTxnId(),fileName);
			if(checkfileName){
				errorListAddUpload.add(RegInitConstant.lbl_reg_init_upload_msg_file_name);
			...
			//if (rule.isError() || size <= 0) 
				...
				if (fileSizeInKB > RegInitConstant.MAX_FILE_SIZE) {
					...
					String insertFileName=commonBo.insertFileName(regForm.getHidnRegTxnId(),fileName);
