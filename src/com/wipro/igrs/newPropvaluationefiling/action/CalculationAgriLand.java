...
public class CalculationAgriLand {
	...
	/*
	 * public double calcDiverted(dto){ //calculation logic for Diverted
	 * calcFloorValue(dto); return value; }
	 * 
	 * public double calcUndiverted (dto){
	 * 
	 * return value; }
	 * 	
	 * public double calcBoth(dto){ //calculation logic for both
	 * calcDiverted(dto); calcUndiverted(dto); return value; }
	 * 
	 * public double calcWithoutSubclauses(dto){ //calculation logic without
	 * subclauses. //based on sub type id, above methods will be called here
	 * return baseValue; } public double calcInternalSubclauses(baseValue,dto){
	 * //calculation logic with screen level subclauses return newBaseValue; }
	 * public double calcWithSubclauses(newBaseValue,Subclause ids){
	 * //calculation logic with subclauses return finalValue; }
	 */
		...
		if (guideLineValuesList != null && guideLineValuesList.size() > 0) {
			for (int i = 0; i < guideLineValuesList.size(); i++) {
				...
				if (i == 0) {
					...
					IrrigatedRate = Double.parseDouble((String) subList.get(1)); // --
																					// --
																					// -
																					// null
																					// handling
			...
			double[][] arr = new double[3][2]; // 2D Double array. [3] for areas
												// & [2] for corresponding rates

			// below code for sorting areas in ascending order of their rates
			if (IrrigatedRate <= OneCropRate && IrrigatedRate <= DoubleCropRate) {

				arr[0][0] = IrrigatedArea;
				arr[0][1] = IrrigatedRate;

				if (OneCropRate <= DoubleCropRate) {
					arr[1][0] = OneCropArea;
					arr[1][1] = OneCropRate;
					arr[2][0] = DoubleCropArea;
					arr[2][1] = DoubleCropRate;
				} else {
					arr[1][0] = DoubleCropArea;
					arr[1][1] = DoubleCropRate;
					arr[2][0] = OneCropArea;
					arr[2][1] = OneCropRate;
				}
				...
				if (IrrigatedRate <= DoubleCropRate) {
					arr[1][0] = IrrigatedArea;
					arr[1][1] = IrrigatedRate;
					arr[2][0] = DoubleCropArea;
					arr[2][1] = DoubleCropRate;
				} else {
					arr[1][0] = DoubleCropArea;
					arr[1][1] = DoubleCropRate;
					arr[2][0] = IrrigatedArea;
					arr[2][1] = IrrigatedRate;
				}
			} else if (DoubleCropRate <= IrrigatedRate && DoubleCropRate <= OneCropRate) {
				arr[0][0] = DoubleCropArea;
				arr[0][1] = DoubleCropRate;
			...
			if (propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("1") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("2") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("3") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("4")) {
				...
				if (propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("1")) {
					...
					if (TotalArea < (x / 10000)) {
						...
						// commneted by ankit for new slab changes given by
						// dept.
						// value=
						// value+(x*ResiRate)+arr[0][0]*arr[0][1]+arr[1][0
				...
				// value=Double.parseDouble(undivpropDTO.
				// getAgriUndivSingleCropArea())*UnIrrigatedRate +
				// 1.25*Double.parseDouble
				// (undivpropDTO.getAgriUndivDoubleCropArea())*UnIrrigatedRate +
				// Double
				// .parseDouble(undivpropDTO.getAgriUndivDoubleCropArea())*
				// IrrigatedRate;
			} else {
	...
	public String[] calcDiverted(PropertyValuationDTO divpropDTO, PropertyValuationDTO propDTO) {
		...
		double constructedArea = Double.parseDouble(propDTO.getConstructedAreaBuilding());// in
																							// hectares
		...
		arr[0][0] = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "26")); // Resi
																				// rate
		arr[0][1] = resiArea; // area

		arr[1][0] = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "27")); // commercial
																				// rate
		arr[1][1] = commArea; // area

		arr[2][0] = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "28")); // industrail
																				// rate
		...
		if (propDTO.getSubAreaId().equalsIgnoreCase("6")) {
			...
			if ("YESEDUC".equalsIgnoreCase(propDTO.getIsAgriTncpEducational())) {
				arr[3][0] = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "30")); // edu
																						// rate
				arr[3][1] = schoolArea; // area
			} else {
			...
			if ("YESHEALTH".equalsIgnoreCase(propDTO.getIsAgriTncpHealth())) {
				arr[4][0] = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "29")); // health
																						// rate
				arr[4][1] = healthArea;
			} else {
		...
		if (propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("1") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("2") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("3") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("4")) {
			...
			if (totalArea <= (x / 10000)) {
				...
				for (int i = arr.length - 1; i >= 0; i--) {
					...
					/*
					 * if(tempId==1){ Value = Value
					 * +calValSlab4_1(arr[i][1]10000,arr[i][0]); } else
					 * if(tempId==2){ Value = Value
					 * +calValSlab4_2(arr[i][1]10000,arr[i][0]); }else{ Value =
					 * Value +calValSlab4_4(arr[i][1]10000,arr[i][0]); }
					 */
				...
				/*
				 * arr[0][0]=Double.parseDouble(propDAO.getL1Rate(propDTO, "1",
				 * "1"));
				 * arr[1][0]=Double.parseDouble(propDAO.getL1Rate(propDTO, "2",
				 * "1"));
				 * arr[2][0]=Double.parseDouble(propDAO.getL1Rate(propDTO, "3",
				 * "1")); if("5".equalsIgnoreCase(propDTO.getSubAreaId())) {
				 * arr[3][0]=Double.parseDouble(propDAO.getL1Rate(propDTO, "4",
				 * "1"));
				 * arr[4][0]=Double.parseDouble(propDAO.getL1Rate(propDTO, "5",
				 * "1")); } else { arr[3][0]=arr[0][0]; arr[4][0]=arr[0][0]; }
				 * arr[5][0]=Double.parseDouble(propDAO.getL1Rate(propDTO, "6",
				 * "1")); java.util.Arrays.sort(arr, new
				 * java.util.Comparator<double[]>() { public int
				 * compare(double[] a, double[] b) { return Double.compare(a[0],
				 * b[0]); } });
				 */
				...
				/*
				 * for(int i=arr.length-1;i>=0;i--) { // Value =
				 * Value+arr[i][0]arr[i][1]10000;
				 * 
				 * //added by ankit for new slab changes if(tempId==1){ Value =
				 * Value +calValSlab4_1(arr[i][1]10000,arr[i][0]); } else
				 * if(tempId==2){ Value = Value
				 * +calValSlab4_2(arr[i][1]10000,arr[i][0]); }else{ Value =
				 * Value +calValSlab4_4(arr[i][1]10000,arr[i][0]); }
				 * 
			...
			if (totalArea > (300.00 / 10000.0)) {
				...
				if (propDTO.getSubAreaId().equalsIgnoreCase("6")) {
					...
					if ("YESEDUC".equalsIgnoreCase(propDTO.getIsAgriTncpEducational())) {
						arr[3][0] = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "30")); // resi
																								// rate
						arr[3][1] = schoolArea; // area
					} else {
						arr[3][0] = arr[0][0]; // edu rate
					...
					if ("YESHEALTH".equalsIgnoreCase(propDTO.getIsAgriTncpHealth())) {
						arr[4][0] = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "29")); // resi
																								// rate
						arr[4][1] = healthArea;
					} else {
						arr[4][0] = arr[0][0]; // health rate
				 ...
				 * Value+arr[i][0]arr[i][1]10000; //changes by ankit for new
				 * slab work Value =
				 * Value+calValSlab4_4(arr[i][1]10000,arr[i][0]);
				 * 
				 * }
...

	{
		...
		double constructedArea = Double.parseDouble(bothpropDTO.getAgriBuildingConstructArea()) / 10000; // in
																											// hectares
		...
		if (propDTO.getSubAreaId().equalsIgnoreCase("6")) {
			...
			if ("YESHEALTH".equalsIgnoreCase(propDTO.getIsAgriTncpHealth())) {
				healthrate = Double.parseDouble(propDAO.getL2Rate(propDTO, "22", "29")); // resi
																							// rate
		...
		if (propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("1") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("2") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("3") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("4")) {
			...
			if ((totalDivArea + totalUnDivArea) <= ((x / 10000) - constructedArea)) {
				/*
				 * for(int i=0;i<unDivArray.length;i++) { // unDivValue =
				 * unDivValue+unDivArray[i][0]unDivArray[i][1]; //changes by
				 * ankit for new slab changes if(tempId==1){ unDivValue =
				 * unDivValue+calValSlab4_1(unDivArray[i][1],unDivArray[i][0]);
				 * }else if(tempId==2){ unDivValue =
				 * unDivValue+calValSlab4_2(unDivArray[i][1],unDivArray[i][0]);
				 * }else{ unDivValue =
				 * unDivValue+calValSlab4_4(unDivArray[i][1],unDivArray[i][0]);
				 * }
				 * 
				 * }
				 */
				...
				/*
				 * for(int i=0;i<divArray.length;i++) { //divValue =
				 * divValue+divArray[i][0]divArray[i][1]10000; //changes by
				 * ankit for new slab changes
				 * 
				 * if(tempId==1){ divValue =
				 * divValue+calValSlab4_1(divArray[i][1]10000,divArray[i][0]);
				 * }else if(tempId==2){ divValue =
				 * divValue+calValSlab4_2(divArray[i][1]10000,divArray[i][0]);
				 * }else{ divValue =
				 * divValue+calValSlab4_4(divArray[i][1]10000,divArray[i][0]); }
				 * }
				 */
				...
				if (constructedArea >= (x / 10000)) {
					for (int i = 0; i < unDivArray.length; i++) {
						 ...
						 * if(tempId==1){ unDivValue =
						 * unDivValue+calValSlab4_1(unDivArray
						 * [i][1],unDivArray[i][0]); }else if(tempId==2){
						 * unDivValue =
						 * unDivValue+calValSlab4_2(unDivArray[i][1]
						 * ,unDivArray[i][0]); }else{ unDivValue =
						 * unDivValue+calValSlab4_4
						 * (unDivArray[i][1],unDivArray[i][0]); }
						 */
					...
					if (newX <= totalDivArea) {
						...
						/*
						 * for(int i=0;i<divArray.length;i++) { //divValue =
						 * divValue+divArray[i][0]divArray[i][1]10000; //changes
						 * by ankit for new slab changes if(tempId==1){ divValue
						 * =
						 * divValue+calValSlab4_1(divArray[i][1]10000,divArray[
						 * i][0]); }else if(tempId==2){ divValue =
						 * divValue+calValSlab4_2
						 * (divArray[i][1]10000,divArray[i][0]); }else{ divValue
						 * =
						 * divValue+calValSlab4_4(divArray[i][1]10000,divArray[
						 * i][0]); } }
						 */
						...
						/*
						 * if(tempId==1){ unDivValue =
						 * unDivValue+calValSlab4_1(newX110000,resirate); }else
						 * if(tempId==2){ unDivValue =
						 * unDivValue+calValSlab4_2(newX110000,resirate); }else{
						 * unDivValue =
						 * unDivValue+calValSlab4_4(newX110000,resirate); }
						 */
						double higherRate = 0;
						double tempTotalArea = 0.0;
					...
					/*
					 * double newX=((x/10000)-constructedArea);
					 * if(newX<=totalUnDivArea) { double minusx=newX; for(int
					 * i=0;i<unDivArray.length;i++) {
					 * 
					 * if(minusx<=unDivArray[i][1] && minusx!=0.0 ) {
					 * unDivArray[i][1]=unDivArray[i][1]-minusx; minusx=0.0; }
					 * 
					 * else if(minusx==0.0) {
					 * 
					 * }
					 * 
					 * else { minusx=minusx-unDivArray[i][1];
					 * unDivArray[i][1]=0.0; } } for(int
					 * i=0;i<unDivArray.length;i++) { unDivValue =
					 * unDivValue+unDivArray[i][0]unDivArray[i][1]; }
					 * unDivValue=unDivValue+ newX10000resirate;
					 * divValue=1.5totalDivAreaIrrigatedRate; } else
					 * if(newX>totalUnDivArea) {
					 * 
					 * 
					 * unDivValue=totalUnDivArearesirate10000; double
					 * xForDiv=newX-totalUnDivArea; double minusx=xForDiv;
					 * totalUnDivArea=0; totalDivArea=totalDivArea-xForDiv;
					 * for(int i=divArray.length-1;i>=0;i--) {
					 * if(minusx>=divArray[i][1]) {
					 * minusx=minusx-divArray[i][1]; continue; } else
					 * if(minusx<=divArray[i][1]&&minusx>=0) {
					 * divArray[i][1]=minusx; minusx=0.0; continue; } else
					 * if(minusx<=0) { divArray[i][1]=0.0; } } for(int
					 * i=0;i<divArray.length;i++) { divValue =
					 * divValue+divArray[i][0]divArray[i][1]10000; }
					 * 
					 * divValue = divValue+1.5totalDivAreaIrrigatedRate;
					 * 
					 * 
					 * }
					 */}//
		...
		for (int j = 1; j < ids.length; j++) {
			for (int i = 0; i < guideLineValues.size(); i++) {
				...
				if (ids[j].equalsIgnoreCase(subList.get(0).toString())) {
					 ...
					 * finalValue=baseValue
					 * +(baseValue(Double.valueOf(operand)/100)); }
					 * if(operator.equalsIgnoreCase("-%")){
					 * finalValue=baseValue-
					 * (baseValue(Double.valueOf(operand)/100)); }
					 * if(operator.equalsIgnoreCase("*")){
					 * finalValue=baseValueDouble.valueOf(operand); } }
					 */
	...
	private double majorMineral(PropertyValuationForm propForm) {
		...
		for (int i = 0; i < propForm.getAgriAddBuyerList().size(); i++) {
			...
			if (buyerDTO.getAgriLandTypeId().equalsIgnoreCase("5")) {
				...
				double constructedArea = Double.parseDouble(buyerDTO.getAgriBuildingConstructArea());// in
																										// hectares
				constructedArea = constructedArea / 10000;
			...
			if (buyerDTO.getAgriLandTypeId().equalsIgnoreCase("7")) {
				...
				double schoolArea = Double.parseDouble(buyerDTO.getAgriTotalDivertedEducationalArea());
				double otherArea = Double.parseDouble(buyerDTO.getAgriTotalDivertedOthersArea());
				double constructedArea = Double.parseDouble(buyerDTO.getAgriBuildingConstructArea()) / 10000; // in
																												// hectares
				...
				if ("unopenshare".equalsIgnoreCase(shareFlag)) {
					// double noBuyer
					// =Double.parseDouble(propDTO.getNoOfBuyers());
					totalDivArea = totalDivArea / noBuyer;
					oneCropArea = oneCropArea / noBuyer;
					twoCropArea = twoCropArea / noBuyer;
					irrigatedArea = irrigatedArea / noBuyer;
				...
				/*
				 * java.util.Arrays.sort(unDivArray, new
				 * java.util.Comparator<double[]>() { public int
				 * compare(double[] a, double[] b) { return Double.compare(a[0],
				 * b[0]); } });
				 */
				...
				if (constructedArea > 0 && constructedArea <= totalDivArea) {
					 ...
					 * 
					 * if(minusConstructedArea<=unDivArray[l][1] &&
					 * minusConstructedArea!=0.0 ) {
					 ...
					 * 
					 * else {
					 * minusConstructedArea=minusConstructedArea-unDivArray
					 * [l][1]; unDivArray[l][1]=0.0; } }
					 */}
				if (propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("1") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("2") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("3") || propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("4")) {
					double x = 0;
					if (propDTO.getAgriApplicableSubclauseId().equalsIgnoreCase("1")) {
	...
	public String[] calcTotalTreeValue(TreeDTO dto, PropertyValuationDTO propDTO) {
		double value = 0;
