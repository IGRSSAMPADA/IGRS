package com.wipro.igrs.newPropvaluation.dao;
...
import com.wipro.igrs.newPropvaluation.constant.PropertyValuationConstant; //import com.wipro.igrs.newPropvaluation.dto.KhasraClrDTO;
...
public class PropertyValuationDAO {
	...
	public String getTehsilClrFlag(String language, String tehsilID) {
		...
		try {
			...
			dbUtil.createPreparedStatement(PropertyValuationSQL.TEHSIL_CLR_FLAG_CHANGED);
	...
	public String lgdCode(String colonyId) {
		...
		try {
			...
			dbUtil.createPreparedStatement(PropertyValuationSQL.LGD_CODE);

			lgdCode = dbUtil.executeQry(new String[]{colonyId});
			return lgdCode;
	...
	public boolean submitValuationMain(PropertyValuationDTO propDTO) {
		...
		try {
			...
			String[] arr = new String[]{propDTO.getValuationId(), propDTO.getDistrictId(), propDTO.getTehsilId(), propDTO.getAreaId(), propDTO.getSubAreaId(), propDTO.getWardPatwariId().split("~")[0], propDTO.getColonyId().split("~")[0], propDTO.getPropertyId(), propDTO.getGuideLineId(), propDTO.getGuideLineValue(), propDTO.getFinalMarketValue(), propDTO.getUserId(), propDTO.getMunicipalFlag()};
	...
	public boolean submitMultiStoreyDetails(PropertyValuationDTO propDTO, String userId) {
		...
		try {
			...
			if (flag1) {
				...
				String olderID = String.valueOf(propDTO.getOlderId());
				String onlyResidential = "";
				if (propDTO.getMultiStoreyTypeId().equalsIgnoreCase("18")) {

					onlyResidential = "R";
				}

				String[] arr = new String[]{propDTO.getBuildingTxnId(), valuationId, propDTO.getBuildingTypeId(), propDTO.getFloorArea(), propDTO.getBuiltUpArea(), propDTO.getCommonArea(), propDTO.getFloorId(), older20, older50, propDTO.getIsLift(), "YES".equalsIgnoreCase(propDTO.getIsOpenTerraceFlag()) ? "Y" : "N", propDTO.getOpenTerraceArea(), propDTO.getOpenTerraceUsageId(), propDTO.getMultiCommPropId(), propDTO.getIsLift(), propDTO.getNearRoad(), propDTO.getUserId(), propDTO.getMultiStoreyTypeId(), olderID, onlyResidential};
				flag2 = dbUtil.executeUpdate(arr);
			}
			if (flag1 && flag2) {
	...
	public boolean submitIndependentBuildingDetails(PropertyValuationDTO propDTO) {
		...
		try {
			...
			if (flag1) {
				...
				String[] arr = new String[]{buildingID, valuationId, propDTO.getBuildingTypeId(), propDTO.getPlotTotArea(), propDTO.getPlotResiArea(), propDTO.getPlotCommArea(), propDTO.getPlotIndusArea(), propDTO.getPlotSchoolArea(), propDTO.getPlotHealthArea(), propDTO.getPlotOtherArea(), propDTO.getIsTncpSchool(), propDTO.getIsTncpHealth(), propDTO.getPlotCost(), propDTO.getConstructionCost(), propDTO.getUserId(), propDTO.getIsAkvnFlag(), propDTO.getIsSuperConstructionFlag(), propDTO.getHousingCheckFlag(), propDTO.getResiCommArea(), olderId, onlyResidential};
					...
					for (int i = 0; i < propDTO.getFloorMarketValueList().size(); i++) {
						...
						if (boo) {
							...
							if (propDTO.getIndFloorAreaList() != null && propDTO.getIndFloorAreaList().size() > 0) {
								...
								dbUtil = new DBUtility();
								dbUtil.createPreparedStatement(PropertyValuationSQL.INSERT_FLOOR_AREA_DETAILS_INDUS);// changed for storing floor area
								String[] arr2 = new String[]{floortxnid, "10", indDTO.getRccArea(), indDTO.getRbcArea(), indDTO.getTinArea(), indDTO.getKacchaArea(), indDTO.getShopArea(), indDTO.getOfficeArea(), indDTO.getGodownArea(), propDTO.getUserId(), propDTO.getIndustrialConstArea()};
								boo = dbUtil.executeUpdate(arr2);
								if (!boo) {
									dbUtil.rollback();
									break;
	...
	// added by Shreeraj for Multistorey Commercial Changes
	public String getFloorRebate(String floorId, String l1Id, String rebateType) {
		String rebate = "";
		try {
			dbUtil = new DBUtility();
		} catch (Exception e1) {
		...
		try {
			...
			dbUtil.createPreparedStatement(PropertyValuationSQL.FLOOR_REBATE_ADV);
			rebate = dbUtil.executeQry(new String[]{floorId, l1Id, rebateType});
			return rebate;
	...
	public ArrayList getAgriLandType(String language, String deedId, String instId) {
		...
		try {
			dbUtil.createStatement();
			if (language.equalsIgnoreCase("en")) {
				if ("1092".equalsIgnoreCase(deedId) && !("2214".equalsIgnoreCase(instId) || "2215".equalsIgnoreCase(instId))) {// changed
					// by
					// akansha
					// to
					// fix
				...
				else if ("2214".equalsIgnoreCase(instId) || "2215".equalsIgnoreCase(instId)) {// changed
					// by
					// akansha
					// to
					// fix
					...
					// 07
					// 2020
					agriLandTypeList = dbUtil.executeQuery(PropertyValuationSQL.AGRILAND_TYPE_NAME_EN_AGI_lease);
				...
				if ("1092".equalsIgnoreCase(deedId) && !("2214".equalsIgnoreCase(instId) || "2215".equalsIgnoreCase(instId))) {// changed
					...
					// 07
					// 2020
					agriLandTypeList = dbUtil.executeQuery(PropertyValuationSQL.AGRILAND_TYPE_NAME_HI_AGI_lease);
	...
	public boolean insertBuyerTreeDetails(TreeDTO dto, PropertyValuationForm propForm, String language, String userId) throws Exception {
		...
		String arrAgriTxnDetls[] = new String[18];
		...
		{
			arrAgriTxnDetls[10] = "Y";// changed by akansha on21st may 2018
		} else {
		...
		for (int i = 0; i < propForm.getAgriAddBuyerList().size(); i++) {
			...
			if (buyerDTO.getAgriLandTypeId().equalsIgnoreCase("5")) {
				...
				try {
					...
					/*if (updated == true && propForm.getPropDTO().getClrFlag() != null && clrLoop == 1) {
						if (propForm.getPropDTO().getPropertyId().equalsIgnoreCase("3") && propForm.getPropDTO().getClrFlag().equalsIgnoreCase("Y")) {
							for (int j = 0; j < propForm.getSampadaClrKhasraList().size(); j++) {
								if(dbUtil.isClosed()){
									logger.debug("Opening closed connection....");
									dbUtil = new DBUtility();
								}
								dbUtil.createPreparedStatement(PropertyValuationSQL.INSERT_UNDIV_AGRI_KHASRA_CLR_DETLS);

								String khasraClrArray[] = new String[32]; // changed
								// by
								// akansha
								// for
								// lgdcode
								...
								khasraClrArray[2] = propForm.getSampadaClrKhasraList().get(j).getKhasraNoClr();
								khasraClrArray[3] = propForm.getSampadaClrKhasraList().get(j).getClrKhasraId();
								khasraClrArray[4] = propForm.getSampadaClrKhasraList().get(j).getKhasraTotalArea();
								khasraClrArray[5] = propForm.getSampadaClrKhasraList().get(j).getLandIrriOrNot();
								khasraClrArray[6] = propForm.getSampadaClrKhasraList().get(j).getIrrigatedArea();
								khasraClrArray[7] = propForm.getSampadaClrKhasraList().get(j).getLandUnIrriOrNot();

								// khasraClrArray[8] = propForm.getClrDto().getNonIrrigatedArea();
								khasraClrArray[8] = propForm.getSampadaClrKhasraList().get(j).getSingleCropArea();
								khasraClrArray[9] = propForm.getSampadaClrKhasraList().get(j).getLandUnirriDouble();
								khasraClrArray[10] = propForm.getSampadaClrKhasraList().get(j).getDoubleCropArea();
								khasraClrArray[11] = propForm.getSampadaClrKhasraList().get(j).getPadtiFlag();
								khasraClrArray[12] = propForm.getSampadaClrKhasraList().get(j).getPadtiArea();
								khasraClrArray[13] = propForm.getSampadaClrKhasraList().get(j).getOwnership();
								khasraClrArray[14] = propForm.getSampadaClrKhasraList().get(j).getNohiyat();
								khasraClrArray[15] = propForm.getSampadaClrKhasraList().get(j).getCurrentStatus();
								khasraClrArray[16] = propForm.getSampadaClrKhasraList().get(j).getKehfiyat();

								khasraClrArray[17] = propForm.getSampadaClrKhasraList().get(j).getClrFlag();
								...
								khasraClrArray[19] = propForm.getSampadaClrKhasraList().get(j).getMortgage();
								...
								// Below change by Ajit for CLR for Undiverted
								// Agriland Bug 10 start
								...
								// change by Ajit for CLR for Undiverted
								// Agriland Bug 10 start
								khasraClrArray[27] = "Y";
								khasraClrArray[28] = propForm.getSampadaClrKhasraList().get(j).getLgdcode();
								khasraClrArray[29] = propForm.getSampadaClrKhasraList().get(j).getLandParcelId();
								khasraClrArray[30] = propForm.getSampadaClrKhasraList().get(j).getLandUseType();
								khasraClrArray[31] = propForm.getSampadaClrKhasraList().get(j).getLandRevenue();
								boolean flag = dbUtil.executeUpdate(khasraClrArray);
								logger.debug("CLR Khasra khasra Data insertion flag" + flag);

								if (flag == true) {
									dbUtil.commit();
									insertClrOtherDetail(propForm.getSampadaClrKhasraList().get(j), khasraTxnID);
								} else {
									dbUtil.rollback();
								}
							}
							...
							// boolean updatedNew = dbUtil
							// .executeUpdate(khasraClrArray);
						}

					}*/
					
					if (updated == true && propForm.getPropDTO().getClrChangedFlag() != null && clrLoop == 1) {
						
						

						if (propForm.getPropDTO().getPropertyId().equalsIgnoreCase("3") && propForm.getPropDTO().getClrChangedFlag().equalsIgnoreCase("Y")) {
							for (int j = 0; j < propForm.getSampadaClrKhasraList().size(); j++) {
								if(dbUtil.isClosed()){
									logger.debug("Opening closed connection....");
									dbUtil = new DBUtility();
								}
								dbUtil.createPreparedStatement(PropertyValuationSQL.INSERT_UNDIV_AGRI_KHASRA_CLR_DETLS_updated);

								String khasraClrArray[] = new String[34]; // changed
								// by
								// akansha
								// for
								// lgdcode
								...
								khasraClrArray[3] = propForm.getSampadaClrKhasraList().get(j).getClrKhasraId();
								khasraClrArray[4] = propForm.getSampadaClrKhasraList().get(j).getKhasraTotalArea();
								khasraClrArray[5] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getRiCircleClr();
								khasraClrArray[6]= propForm.getSampadaClrKhasraList().get(j).getClrChangedFlag();
								khasraClrArray[8] = userId;
								khasraClrArray[9] =propForm.getPropDTO().getRinPustikaNoClr();
								khasraClrArray[10] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaSingleCrop());
								khasraClrArray[11] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaDoubleCrop());
								khasraClrArray[12] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaIrrigated());
								khasraClrArray[13] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getTotalSaleableArea());
								khasraClrArray[14] = "Y";
								khasraClrArray[15] = propForm.getSampadaClrKhasraList().get(j).getCensusCode();
								khasraClrArray[16] = propForm.getSampadaClrKhasraList().get(j).getLgdcode();
								khasraClrArray[17] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getNoyiyatClr();
								khasraClrArray[18] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getDistCodeClr();
								khasraClrArray[19] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getTehCodeClr();
								khasraClrArray[20] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getHalkaNoClr();
								khasraClrArray[21] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getBasraNumberClr();
								khasraClrArray[22] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getKhasraIdClr();
								khasraClrArray[23] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getActionStatusClr();
								khasraClrArray[24] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandUseClr();
								khasraClrArray[25] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandTypeDesc();
								khasraClrArray[26] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getCropClr();
								khasraClrArray[27] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getUlPinClr();
								khasraClrArray[28] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getAlPinClr();
								khasraClrArray[29] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandTypeClr();
								khasraClrArray[30] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getCropTypeDesc();
								khasraClrArray[31] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getPlotTypeClr();
								khasraClrArray[32] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getPlotTypeDesc();
								...
								khasraClrArray[7] = totalSaleableArea;// String.valueOf(propForm.getSampadaClrKhasraList().get(j).getTotalSaleableArea());
								khasraClrArray[33] = propForm.getSampadaClrKhasraList().get(j).getTransactionType();
								boolean flag = dbUtil.executeUpdate(khasraClrArray);
								logger.debug("CLR Khasra khasra Data insertion flag" + flag);

								if (flag == true) {
									dbUtil.commit();
									insertClrOtherOwnerDetls(propForm.getSampadaClrKhasraList().get(j), khasraTxnID, valTxnId);
								} else {
									dbUtil.rollback();
								}
							}
					...
					if (updated == true && propForm.getPropDTO().getClrChangedFlag() != null && clrLoop == 1) {
						if (propForm.getPropDTO().getPropertyId().equalsIgnoreCase("3") && propForm.getPropDTO().getClrChangedFlag().equalsIgnoreCase("Y")) {
							...
							for (int j = 0; j < propForm.getSampadaClrKhasraList().size(); j++) {
								...
								dbUtil.createPreparedStatement(PropertyValuationSQL.INSERT_DIV_AGRI_KHASRA_CLR_DETLS_Updated);

								// String khasraClrArray[] = new String[26];
								String khasraClrArray[] = new String[37]; // changed
								// by
								// akansha
								// for
								// lgd
								...
								khasraClrArray[7] = totalSaleableArea;// String.valueOf(propForm.getSampadaClrKhasraList().
								
								khasraClrArray[8] = userId;
								khasraClrArray[9] =propForm.getPropDTO().getRinPustikaNoClr();
							
								khasraClrArray[10] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedArea());
								khasraClrArray[11] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedResidentialArea());
								khasraClrArray[12] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedCommercialArea());

								khasraClrArray[13] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedIndustrialArea());
								khasraClrArray[14] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedEducationalArea());
								khasraClrArray[15] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedHealthArea());
								khasraClrArray[16] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedOthersArea());
								

								khasraClrArray[17] = "Y";
								
								khasraClrArray[18] = propForm.getSampadaClrKhasraList().get(j).getCensusCode();
								khasraClrArray[19] = propForm.getSampadaClrKhasraList().get(j).getLgdcode();
								khasraClrArray[20] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getNoyiyatClr();
								khasraClrArray[21] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getDistCodeClr();
								khasraClrArray[22] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getTehCodeClr();
								khasraClrArray[23] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getHalkaNoClr();
								khasraClrArray[24] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getBasraNumberClr();
								khasraClrArray[25] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getKhasraIdClr();
								khasraClrArray[26] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getActionStatusClr();
								khasraClrArray[27] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandUseClr();
								khasraClrArray[28] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandTypeDesc();
								khasraClrArray[29] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getCropClr();
								khasraClrArray[30] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getUlPinClr();
								khasraClrArray[31] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getAlPinClr();
								khasraClrArray[32] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandTypeClr();
								khasraClrArray[33] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getCropTypeDesc();
								khasraClrArray[34] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getPlotTypeClr();
								khasraClrArray[35] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getPlotTypeDesc();
								khasraClrArray[36]=propForm.getSampadaClrKhasraList().get(j).getTransactionType();;
								boolean flag = dbUtil.executeUpdate(khasraClrArray);
			...
			// -----for agriLand both----
				...
				arrBothBuyerDetls[22] = buyerDTO.getIsConstructionKhasraNo();
					...
					if (updated == true && propForm.getPropDTO().getClrChangedFlag() != null && clrLoop == 1) {
						if (propForm.getPropDTO().getPropertyId().equalsIgnoreCase("3") && propForm.getPropDTO().getClrChangedFlag().equalsIgnoreCase("Y")) {
							for (int j = 0; j < propForm.getSampadaClrKhasraList().size(); j++) {
								...
								dbUtil.createPreparedStatement(PropertyValuationSQL.INSERT_BOTH_AGRI_KHASRA_CLR_DETLS_Updated);

								// String khasraClrArray[] = new String[26];
								String khasraClrArray[] = new String[40];// changed
								// by
								// akansha
								// for
								// lgd_code
								...
								khasraClrArray[7] = totalSaleableArea;
								...
								khasraClrArray[15] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedHealthArea());
								khasraClrArray[16] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaTotalDivertedOthersArea());
								khasraClrArray[17] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaSingleCrop());
								khasraClrArray[18] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaDoubleCrop());
								khasraClrArray[19] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getSampadaIrrigated());
							
								khasraClrArray[20] = String.valueOf(propForm.getSampadaClrKhasraList().get(j).getTotalSaleableArea());


								khasraClrArray[21] = "Y";

								khasraClrArray[22] = propForm.getSampadaClrKhasraList().get(j).getCensusCode();
								khasraClrArray[23] = propForm.getSampadaClrKhasraList().get(j).getLgdcode();
								khasraClrArray[24] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getNoyiyatClr();
								khasraClrArray[25] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getDistCodeClr();
								khasraClrArray[26] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getTehCodeClr();
								khasraClrArray[27] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getHalkaNoClr();
								khasraClrArray[28] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getBasraNumberClr();
								khasraClrArray[29] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getKhasraIdClr();
								khasraClrArray[30] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getActionStatusClr();
								khasraClrArray[31] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandUseClr();
								khasraClrArray[32] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandTypeDesc();
								khasraClrArray[33] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getCropClr();
								khasraClrArray[34] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getUlPinClr();
								khasraClrArray[35] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getAlPinClr();
								khasraClrArray[36] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getLandTypeClr();
								khasraClrArray[37] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getCropTypeDesc();
								khasraClrArray[38] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getPlotTypeClr();
								khasraClrArray[39] = propForm.getSampadaClrKhasraList().get(j).getClrListDto().getPlotTypeDesc();
				...
				if (subId == "" && propDTO.getOlderId() != 0) {
					...
					for (int i = 0; i < subIds.length; i++) {
						...
						if (subIds[i] == null || subIds[i].equalsIgnoreCase("")) {
							...
							// clause id record
							dbUtil = new DBUtility();
							// dbUtil.createPreparedStatement(PropertyValuationSQL.INSERT_INTO_SUBCLAUSE_PROP_MAP);
							dbUtil.createPreparedStatement(PropertyValuationSQL.INSERT_INTO_SUBCLAUSE_PROP_MAP_WITH_SLAB_ID);
	...
	/**
	 * Method : getRinPustikaNo Description : Getting all RinPustika No wrt regTxnId
	 * 
	 * @param query
	 *            : String
	...
	public ArrayList getRinPustikaNo(String regTxnId) {
		...
		public ArrayList getKhasraDbLink(String lgdCode) {
			...
			try{
				dbUtil.createPreparedStatement(PropertyValuationSQL.FETCH_DISTRICT_TEHSIL_CLR);
				ArrayList list1 = dbUtil.executeQuery((new String[]{lgdCode}));
				for(int i = 0; i<list1.size(); i++){
					ArrayList list = (ArrayList) list1.get(i);
					districtID= (String)list.get(0);
					tehsilID = (String)list.get(1);
				}
			}catch (Exception e) {
				logger.error(e);
				return null;
			...
			try {
				...
				dbUtil.createPreparedStatement(PropertyValuationSQL.GET_KHASRA_LIST);

				khasraList = dbUtil.executeQuery((new String[]{lgdCode, districtID, tehsilID}));
				return khasraList;
	...
	/**
	 * Auther : Rakesh Soni Method : getRinPustikaNo Description : Getting all RinPustika No wrt regTxnId
	 * 
	 * @param query
	 *            : String
	 * @throws : Exception Returns : ArrayList
	...
	public String getInstrumentClrFlag(String instId) {
		...
		try {
			dbUtil.createPreparedStatement(PropertyValuationSQL.get_inst_clr_flag);
			clrFlag = dbUtil.executeQry(new String[]{instId});
			return clrFlag;
		} catch (Exception e) {
			logger.error(e);
			return null;
	...
	// added by saurav for inseting clr extra detail
	public void insertClrOtherDetail(SampadaKhasraClrDTO sampadaDto, String khasraTxnId) throws Exception {
		String sql = PropertyValuationSQL.INSERT_CROP_DETAIL;
		ArrayList<CropDetailDTO> crpdtolist = sampadaDto.getCropList();
		ArrayList<TreeNewDTO> treeDtolist = sampadaDto.getTreeList();
		ArrayList<MortgageRemark> mortgageRemarkList = sampadaDto.getMortgageRemarkList();
		ArrayList<RmkList> remarkDtoList = sampadaDto.getRemarkList();
		ArrayList<SourceOfIrrigateDTO> sourceOfIrrigationList = sampadaDto.getSourceOfIrrigationList();
		String khasraNumber = sampadaDto.getKhasraNoClr();
		String khasraId = sampadaDto.getKhasraId();
		if (null != crpdtolist) {
			if (crpdtolist.size() > 0) {
				for (int crp = 0; crp < crpdtolist.size(); crp++) {
					CropDetailDTO cropDto = crpdtolist.get(crp);
					...
					arr[3] = "";
					arr[4] = "";
					arr[5] = cropDto.getSeasonType();
					arr[6] = cropDto.getCropType();
					arr[7] = cropDto.getCropArea();
					if (executeInsert(sql, arr)) {
						logger.debug("CLR Crop detail inserted successfully....");
		...
		if (null != treeDtolist) {
			if (crpdtolist.size() > 0) {
				sql = PropertyValuationSQL.INSERT_TREE_DETAIL;
				...
				arr[3] = "";
				arr[4] = "";
				for (int tree = 0; tree < treeDtolist.size(); tree++) {
					TreeNewDTO treeDto = treeDtolist.get(tree);
					arr[5] = treeDto.getTreeName();
					arr[6] = treeDto.getTreeCount();
					if (executeInsert(sql, arr)) {
						logger.debug("CLR Tree detail inserted successfully....");

					}
				}
			}
		}
		if (null != mortgageRemarkList) {
			if (mortgageRemarkList.size() > 0) {
				sql = PropertyValuationSQL.INSERT_MRTG_DETAIL;
				String[] arr = new String[7];
				arr[0] = khasraTxnId;
				arr[1] = khasraId;
				arr[2] = khasraNumber;
				arr[3] = "";
				arr[4] = "";
				for (int mrtg = 0; mrtg < mortgageRemarkList.size(); mrtg++) {
					MortgageRemark mrtgDTO = mortgageRemarkList.get(mrtg);
					arr[5] = mrtgDTO.getMRemark();
					arr[6] = mrtgDTO.getMtype();
					if (executeInsert(sql, arr)) {
						logger.debug("CLR Mortgage detail inserted successfully....");

					}
				}
			}
		}
		if (null != remarkDtoList) {
			if (remarkDtoList.size() > 0) {
				sql = PropertyValuationSQL.INSERT_REMARK_DETAIL;
				...
				arr[3] = "";
				arr[4] = "";
				for (int rmk = 0; rmk < remarkDtoList.size(); rmk++) {
					RmkList remarkDto = remarkDtoList.get(rmk);
					arr[5] = remarkDto.getRemark();
					if (executeInsert(sql, arr)) {
						logger.debug("CLR Mortgage detail inserted successfully....");
		...
		if (null != sourceOfIrrigationList) {
			if (sourceOfIrrigationList.size() > 0) {
				sql = PropertyValuationSQL.INSERT_IRRIGATION_DETAIL;
				String[] arr = new String[6];
				arr[0] = khasraTxnId;
				arr[1] = khasraId;
				arr[2] = khasraNumber;
				arr[3] = "";
				arr[4] = "";
				for (int rmk = 0; rmk < sourceOfIrrigationList.size(); rmk++) {
					SourceOfIrrigateDTO irrigationDto = sourceOfIrrigationList.get(rmk);
					arr[5] = irrigationDto.getSource();
					if (executeInsert(sql, arr)) {
						logger.debug("CLR Mortgage detail inserted successfully....");
	...
	// added by saurav common method for execute insert and update
	public boolean executeInsert(String sql, String[] arr) {
		boolean flag = false;
		try {
			dbUtil = new DBUtility();
			dbUtil.createPreparedStatement(sql);
	...
	public ArrayList getKhasraDetailsFromDbLink(String khasraid, String lgdCode) {
		...
		try {
				...
				dbUtil.createPreparedStatement(PropertyValuationSQL.get_khasra_detls_list_clr);
			
				khasraList = dbUtil.executeQuery(new String[]{lgdCode,khasraid, districtID, tehsilID});
	...
	public ArrayList OwnerDetlsDTO(String khasraid, String lgdCode) {
		...
		try {
				...
				dbUtil.createPreparedStatement(PropertyValuationSQL.get_owner_details_clr);
...
public void insertClrOtherOwnerDetls(SampadaKhasraClrDTO sampadaDto, String khasraTxnId, String valTxnId) throws Exception {
	String sql = PropertyValuationSQL.INSERT_OWNER_DETLS_CLR_UPDATED;
	ArrayList<OwnerDetlsDTO> owndtolist = sampadaDto.getOwnerDTO();
	String khasraNumber = sampadaDto.getKhasraNoClr();
	String khasraId = sampadaDto.getKhasraId();
	if (null != owndtolist) {
		if (owndtolist.size() > 0) {
			for (int own = 0; own < owndtolist.size(); own++) {
				OwnerDetlsDTO ownDTO = owndtolist.get(own);
				String[] arr = new String[10];
				arr[0] = getClrPart_TXN_ID();
				arr[1] = khasraNumber;
				arr[2] = khasraId;
				arr[3] = ownDTO.getOwnername();
				arr[4] = "";
				arr[5] = ownDTO.getOwnerShare();
				arr[6] = ownDTO.getOwnerType();
				arr[7] = ownDTO.getOwnerId();
				arr[8] = valTxnId;
				arr[9] = ownDTO.getFatherName();
				if (executeInsert(sql, arr)) {
					logger.debug("CLR owner detail inserted successfully....");
...
public String getClrPart_TXN_ID() throws Exception {
	...
	String sql = PropertyValuationSQL.CLR_PARTY_TXN_ID;
	// String sqlDate=PropertyValuationSQL.PLOT_SYS_DATE;
	dbUtil.createStatement();
	String seq = dbUtil.executeQry(sql);
	// String currDate=dbUtil.executeQry(sqlDate);
...
public String getLandDesc(String landId) {

	String landDesc = null;
	try {
		dbUtil = new DBUtility();
	} catch (Exception e1) {
		// TODO Auto-generated catch block
	...
	try {
			...
			dbUtil.createPreparedStatement(PropertyValuationSQL.Land_Desc);
		
			landDesc = dbUtil.executeQry(new String[]{landId});
...
public String getPlotDesc(String plotid) {

	String PlotDesc = null;
	try {
		dbUtil = new DBUtility();
	} catch (Exception e1) {
		// TODO Auto-generated catch block
	...
	try {
			...
			dbUtil.createPreparedStatement(PropertyValuationSQL.Plot_Desc);
		
			PlotDesc = dbUtil.executeQry(new String[]{plotid});
...
public String getCropDesc(String cropid) {

	String cropDesc = null;
	try {
		dbUtil = new DBUtility();
	} catch (Exception e1) {
		// TODO Auto-generated catch block
	...
	try {
			...
			dbUtil.createPreparedStatement(PropertyValuationSQL.Crop_Desc);
		
			cropDesc = dbUtil.executeQry(new String[]{cropid});
