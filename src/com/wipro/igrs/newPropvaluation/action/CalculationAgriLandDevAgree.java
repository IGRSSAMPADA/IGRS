...
public class CalculationAgriLandDevAgree {
	...
	public CalculationAgriLandDevAgree() throws Exception
	{
		...
		public String[] calcUndivertedDevAgree(PropertyValuationDTO undivpropDTO, PropertyValuationDTO propDTO) {
			...
			if(guideLineValuesList!=null && guideLineValuesList.size()>0)
			{
				for (int i = 0; i < guideLineValuesList.size(); i++)
				{
					...
					if(i==2)
					{
						...
						DoubleCropRate=Double.parseDouble((String) subList.get(1)); //for double crop rate change
					}
		...
		//------end of addition----
			...
			double remainArea= 0.0;
			double share = propDTO.getBuilderShare();
			
			TotalArea = (share * TotalArea)/100;
			...
			if(IrrigatedRate<=OneCropRate && IrrigatedRate<=DoubleCropRate){
				...
				if(IrrigatedArea<= TotalArea){
					IrrigatedArea = IrrigatedArea;
					remainArea = TotalArea - IrrigatedArea;
					if(OneCropRate<=DoubleCropRate){
						if(OneCropArea<=remainArea){
							DoubleCropArea = remainArea -OneCropArea;
						}else{
							 OneCropArea = remainArea;
							 DoubleCropArea = 0;
						}
						
					}else{
							if(DoubleCropArea<=remainArea){
								OneCropArea = remainArea -DoubleCropArea;
							}else{
								 OneCropArea = 0;
								 DoubleCropArea = remainArea;
							}
					}
					
				}else{
						IrrigatedArea = TotalArea ;
						OneCropArea = 0;
						DoubleCropArea = 0;
				}
				...
				if(OneCropArea<= TotalArea){
					OneCropArea = OneCropArea;
					remainArea = TotalArea - OneCropArea;
					if(IrrigatedRate<=DoubleCropRate){
						if(IrrigatedArea<=remainArea){
							DoubleCropArea = remainArea -IrrigatedArea;
						}else{
								IrrigatedArea = remainArea;
								DoubleCropArea = 0;
						}
						
					}else{
							if(DoubleCropArea<=remainArea){
								IrrigatedArea = remainArea -DoubleCropArea;
							}else{
									IrrigatedArea = 0;
									DoubleCropArea = remainArea;
							}
				...
				if(DoubleCropRate<= TotalArea){
					DoubleCropArea = DoubleCropArea;
					remainArea = TotalArea - DoubleCropArea;
					if(IrrigatedRate<=OneCropRate){
						if(IrrigatedArea<=remainArea){
							OneCropArea = remainArea -IrrigatedArea;
						}else{
								IrrigatedArea = remainArea;
								OneCropArea = 0;
						}
							...
							if(OneCropArea<=remainArea){
								IrrigatedArea = remainArea -OneCropArea;
							}else{
									IrrigatedArea = 0;
									OneCropArea = remainArea;
							}
		...
		public String[] calcDivertedDevAgree(PropertyValuationDTO divpropDTO, PropertyValuationDTO propDTO){
            ...
            if(guideLineValuesList!=null && guideLineValuesList.size()>0)
			{
				for (int i = 0; i < guideLineValuesList.size(); i++)
				{
					...
					{
						OneCropRate=Double.parseDouble((String) subList.get(1));
						DoubleCropRate=Double.parseDouble((String) subList.get(1)); //for double crop rate changes
					}
		...
		public  String[] calculateBothDevAgree(PropertyValuationDTO bothpropDTO, PropertyValuationDTO propDTO)
	      
		{
	          ...
	          //---null handling of undiverted areas
				...
				//below code for sorting areas in ascending order of their rates	            
	            double totalArea = 0;
	            totalArea = totalUnDivArea+totalDivArea;
	            ...
	            totalArea = (share * totalArea)/100;
	            if(totalDivArea<=totalArea){
	            	totalUnDivArea = totalArea-totalDivArea;            	
	            	...
	            	totalUnDivArea = 0;
		             ...
		             totalDivArea = totalArea;
					if(commArea<= totalDivArea){
					
					remainArea = totalDivArea - commArea;
					commArea = commArea;
					resiArea = remainArea;
					indArea =0;
					schoolArea =0;
	            ...
	            if(IrrigatedRate>=OneCropRate && IrrigatedRate>=DoubleCropRate){
					...
					if(irrigatedArea<= totalUnDivArea){
						irrigatedArea = irrigatedArea;
						remainArea = totalUnDivArea - irrigatedArea;
						if(OneCropRate>=DoubleCropRate){
							if(oneCropArea<=remainArea){
								twoCropArea = remainArea -oneCropArea;
							}else{
								 oneCropArea = remainArea;
								 twoCropArea = 0;
							}
								...
								if(twoCropArea<=remainArea){
									oneCropArea = remainArea -twoCropArea;
								}else{
									 oneCropArea = 0;
									 twoCropArea = remainArea;
								}
						}
						
					}else{
							irrigatedArea = totalUnDivArea ;
							oneCropArea = 0;
							twoCropArea = 0;
					}
					...
					if(oneCropArea<= totalUnDivArea){
						oneCropArea = oneCropArea;
						remainArea = totalUnDivArea - oneCropArea;
						if(IrrigatedRate>=DoubleCropRate){
							if(irrigatedArea<=remainArea){
								twoCropArea = remainArea -irrigatedArea;
							}else{
									irrigatedArea = remainArea;
									twoCropArea = 0;
							}
								...
								if(twoCropArea<=remainArea){
									irrigatedArea = remainArea -twoCropArea;
								}else{
										irrigatedArea = 0;
										twoCropArea = remainArea;
								}
					...
					if(twoCropArea<= totalUnDivArea){ //changes by saurav
						twoCropArea = twoCropArea;
						remainArea = totalUnDivArea - twoCropArea;
						if(IrrigatedRate>=OneCropRate){
							if(irrigatedArea<=remainArea){
								oneCropArea = remainArea -irrigatedArea;
							}else{
