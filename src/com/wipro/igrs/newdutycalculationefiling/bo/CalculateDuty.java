...
import com.wipro.igrs.newreginitefiling.bd.RegCommonBD;
...
String govMarketValue=	dutyDTO.getGovValue();
	...
	{
	stampDetails=dao.getStampDutyDetails(String.valueOf(1053),String.valueOf(2090));  //same as conveyance
	...
	if(stampDetails!=null && stampDetails.size()>0)
	{
		...
		switch(operationId)
		{
			case 1:		stampduty=operationValue;
						break;
			case 2:		
			...
			case 3: 	
						...
						break;
			case 4:		if(marketValue>considerationAmount)
						{
						...
						break;
			case 5:		if(instDTO.getCommonUserField()!=null && !instDTO.getCommonUserField().equalsIgnoreCase(""))
						{
						multiply=Double.parseDouble(instDTO.getCommonUserField());
						}
						else if(instDTO.getInStampDutyAmount()!=null && !instDTO.getInStampDutyAmount().equalsIgnoreCase(""))
						...
						break;
			case 6:		 if(instDTO.getInStampDutyAmount()!=null && !instDTO.getInStampDutyAmount().equalsIgnoreCase(""))
						{
						multiply=Double.parseDouble(instDTO.getInStampDutyAmount());
						}
	...
	else
	{
		if(instDTO.getInstId()==2179)
		{
			if("Y".equalsIgnoreCase(dutyDTO.getHvShare()))
			{
				...
				stampduty=(stampAmount*5*marketValue)/100;
...
public String calculateStampDutyCoveyance1Percent(InstrumentsDTO instDTO,DutyCalculationDTO dutyDTO,DutyCalculationDTO propDTO)
{
	...
	public double calculateMuncipalDuty(InstrumentsDTO instDTO,DutyCalculationDTO dutyDTO,DutyCalculationDTO propDTO)
	{
		...
		
		{
			...
			if(municpal!=null && municpal.size()>0)
			{
				...
				else if(("1".equalsIgnoreCase(municipalId)||"5".equalsIgnoreCase(municipalId)||"2".equalsIgnoreCase(municipalId)||"4".equalsIgnoreCase(municipalId))&&"Y".equalsIgnoreCase(flag))
				{
					applicableFlag=true;
				}
				else
			...
			if(applicableFlag)
				{
					...
					if(subList!=null && subList.size()>0)
					{
						...
						{
							//municipalValue=1;
							//changes by preeti on 3 august 2016
							municipalValue=2;
						}
	...
	public double calculateJanPadDuty(InstrumentsDTO instDTO,DutyCalculationDTO dutyDTO,DutyCalculationDTO propDTO)
	{
		...
		if(subList!=null && subList.size()>0)
		{
			...
			switch(operationId)
			{
				...
				case	1:		if(instDTO.getInstId()==2055)
								{
									janPadDuty=500;
								}
							break;
	...
	public String calculateStampDutyConsnter(int instid,int deedid, String considerationAmount,String cancellationFlag,String familyFlag)
	{
		...
		else
		{
			...
			/*else if(instDTO.getInstId()==2091)
			{
				double stampduty1;
				double stampDuty2;
				stampduty1=(marketValue*5)/100;
	...
	public boolean insertAllDutiesConsenter(String regInitId)
	{
		...
		if(consenterList!=null&&consenterList.size()>0)
		{
			...
			for(int i=0;i<1;i++)//hard coded 1 by roopam on 17 april 15 as per departments requirement
			{	
	...
	public int calculatePropWiseExemptedDutiesEWS(InstrumentsDTO instDTO,DutyCalculationDTO dutyDTO)
	{
		...
		for(int i=0;i<exempList.size();i++)
		{
			DutyCalculationDTO dto1=(DutyCalculationDTO) exempList.get(i);
				...
				if(dto1.getExempId().equalsIgnoreCase("131") 
						|| dto1.getExempId().equalsIgnoreCase("132") 
						|| dto1.getExempId().equalsIgnoreCase("133"))
				{
					EWSExemp=true;
		...
		if(EWSExemp){
			...
			if(propertyList!=null && propertyList.size()>0)
			{
				...
				/*dutyDTO.setTotalStampDuty(0);
				dutyDTO.setTotalUpkar(0);
				dutyDTO.setTotalNagarPalika(0);
				dutyDTO.setTotalPanchyat(0);*/
				...
				if("1".equalsIgnoreCase(propId)){//plot
					...
					if(mainList!=null && mainList.size()==1){
						...
						if(propSubId.equalsIgnoreCase("2") ){ //Independent floor 
							
							areaToBeCompared=Double.parseDouble(list.get(1).toString());//total floor area
							
							
						}else if(propSubId.equalsIgnoreCase("3") && list.get(2).toString().equalsIgnoreCase("18")){//Multistorey building-residential
							
							areaToBeCompared=Double.parseDouble(list.get(3).toString());//build up area
							
						}else if(propSubId.equalsIgnoreCase("1")){// Independent building
							...
							if(floorArea!=null && !floorArea.equalsIgnoreCase("")){
								areaToBeCompared=Double.parseDouble(floorArea);//sum of rcc rbc tin and kacha
							}else{
								areaToBeCompared=0;
							}
							
							if(areaToBeCompared>0 && ( (ewsSelected && areaToBeCompared<=30) ||
									(ligSelected && areaToBeCompared<=48)  ||
									(migSelected && areaToBeCompared<=85)) ){
								...
								if(list.get(4)!=null && list.get(4).toString().equalsIgnoreCase("Y")){
									...
									//return 1;//prompt user for checkbox selection
									areaToBeCompared=0;//if checkbox not checked, no exemption to be applied on this property-done on 24May2015
				...
				/*dutyDTO.setTotalStampDuty(dutyDTO.getTotalStampDuty()+ddto.getStampDuty());
				dutyDTO.setTotalUpkar(dutyDTO.getTotalUpkar()+ddto.getUpkarDuty());
				dutyDTO.setTotalNagarPalika(dutyDTO.getTotalNagarPalika()+ddto.getNagarPalikaDuty());
				dutyDTO.setTotalPanchyat(dutyDTO.getTotalPanchyat()+ddto.getPanchayatDuty());*/
		...
		for(int i=0;i<exempList.size();i++)
		{
			rowList=(ArrayList) exempList.get(i);
			 ...
			 currExemp=rowList.get(0)!=null?rowList.get(0).toString():"";
				...
				if(currExemp.equalsIgnoreCase("131") 
						|| currExemp.equalsIgnoreCase("132") 
						|| currExemp.equalsIgnoreCase("133"))
				{
					EWSExemp=true;
			...
			if(propValList!=null && propValList.size()>0)
			{
				...
				if(EWSExemp){
					...
					if(ewsSelected || ligSelected){
						...
						dutiesParam[1] =  myformatter.format(exempAmount);// duty exemp
