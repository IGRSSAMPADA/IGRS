...
public class AddPartyTypeMasterToDB extends BaseAction {
    
	PartyTypeMasterBD partyTypeMasterBD = new PartyTypeMasterBD();

    
    public AddPartyTypeMasterToDB() {
    }
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
    	...
    	partyTypeMasterDTO.setCreatedBy(userId);
    	...
    	boolean b = false;
		try {
			b = partyTypeMasterBD.addPartyTypeMaster(partyTypeMasterDTO,roleId,funId,userId);
    	...
    	if(b)
    		return mapping.findForward("addSuccessPartyPage");
    	else 
